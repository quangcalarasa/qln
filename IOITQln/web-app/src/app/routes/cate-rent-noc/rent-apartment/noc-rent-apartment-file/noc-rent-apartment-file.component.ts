...
import { NocApartmentFileRepository } from 'src/app/infrastructure/repositories/NocApartmentFile.reposityory';
...
@Component({
  selector: 'app-noc-rent-apartment-file',
  templateUrl: './noc-rent-apartment-file.component.html',
  styles: [
  ]
})
export class NocRentApartmentFileComponent implements OnInit {
  ...
  constructor(
    private nocApartmentFileRepository: NocApartmentFileRepository,
  ...
  ngOnInit(): void {
    this.validateForm = this.fb.group({
      ...
      NocApartmentId: [this.record ? this.record.Id : undefined],
  ...
  async submitForm() {
    ...
    const resp = await this.nocApartmentFileRepository.addNew(data);
  ...
  async getData() {
    ...
    this.paging.query = `NocApartmentId=${this.record.Id}`;
    this.paging.order_by = 'CreatedAt Desc';
    try {
      this.loading = true;
      const resp = await this.nocApartmentFileRepository.getByPage(this.paging);
  ...
  async delete(data: any) {
    const resp = await this.nocApartmentFileRepository.delete(data);
    if (resp.meta?.error_code == 200) {
