...
import { AddOrUpdateSalaryComponent } from './add-or-update-salary/add-or-update-salary.component';
...
export class SalaryComponent implements OnInit {
    ...
    { title: 'Diễn giải', index: 'Note' },
    {
      ...
      buttons: [
        ...
        },
        {
          ...
          pop: {
            title: 'Bạn có chắc chắn muốn hệ số lương cơ bản này?',
            okType: 'danger',
            icon: 'star'
          },
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.salaryRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    const drawerRef = this.drawerService.create<AddOrUpdateSalaryComponent>({
      nzTitle: record ? `Sửa hệ số lương cơ bản` : 'Thêm mới hệ số lương cơ bản',
      // record.khoa_chinh
      nzWidth: '55vw',
      nzContent: AddOrUpdateSalaryComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
    ...
    drawerRef.afterClose.subscribe((data: any) => {
      if (data) {
        let msg = data.Id ? `Sửa hệ số lương cơ bản thành công!` : `Thêm hệ số lương cơ bản thành công!`;
  ...
  async delete(data: any) {
    const resp = await this.salaryRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.NocSalary
        }
    });
  }

  async csvExport() {
    const resp = await this.salaryRepository.ExportExcel();
  }
}
