...
import { GeneralDataImportRepository } from 'src/app/infrastructure/repositories/general-data-import.repository';

@Component({
  selector: 'app-data-import-noc',
  templateUrl: './noc.component.html',
  styles: []
})
export class DataImportNocComponent implements OnInit {
  ...
  constructor(private uploadRepository: UploadRepository, 
    private importHistoryRepository: ImportHistoryRepository, private generalDataImportRepository: GeneralDataImportRepository) { }
  ...
  async getData() {
    ...
    const resp = await this.importHistoryRepository.getByPage(this.paging, ImportHistoryTypeEnum.Noc_GeneralDataImport);
  ...
  async handleChange(file: any) {
    ...
    this.loading = true;
    const resp_ren = await this.generalDataImportRepository.importDataExcel(formData);
  ...
  downloadTemplate() {
    this.uploadRepository.downloadFileExcelTemplate(ImportHistoryTypeEnum.Noc_GeneralDataImport);
  }
