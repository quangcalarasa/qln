...
import { AddOrUpdateLandPriceComponent } from './add-or-update/add-or-update-land-price.component';
...
import { TypeDecree, LandPriceType, ImportHistoryTypeEnum, AccessKey } from 'src/app/shared/utils/enums';
...
export class LandPriceComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        { title: 'Giá đất ở tối thiểu', render: 'minValueClmn', className: 'text-right' },
    ...
    async getData() {
        this.paging.query = `LandPriceType=${LandPriceType.NOC}`;
        this.paging.order_by = 'CreatedAt Desc';
        this.paging.landPriceType = LandPriceType.NOC;
        ...
        try {
            this.loading = true;
            const resp = await this.landPriceRepository.getByPage(this.paging, this.query.txtSearch);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdateLandPriceComponent>({
            nzTitle: record ? `Sửa thông tin giá đất` : 'Thêm mới thông tin giá đất',
            // record.khoa_chinh
            nzWidth: '75vw',
            nzContent: AddOrUpdateLandPriceComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
    ...
    async delete(data: any) {
        const resp = await this.landPriceRepository.delete(data);
        if (resp.meta?.error_code == 200) {
    ...
    async getDataDecreeType2() {
        let paging: GetByPageLandPriceModel = new GetByPageLandPriceModel();
        paging.page_size = 0;
    ...
    import() {
        const drawerRef = this.drawerService.create<SharedImportExcelComponent>({
            nzTitle: `Import excel bảng giá đất cho nhà ở cũ`,
            nzWidth: '85vw',
            nzPlacement: 'left',
            nzContent: SharedImportExcelComponent,
            nzContentParams: {
                importHistoryType: ImportHistoryTypeEnum.Noc_Landprice
            }
        });
