...
import { AddorupdateMd167HouseTypeComponent } from './addorupdate/addorupdate.component';
...
export class HouseTypeComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: 'Mã loại nhà', index: 'Code' },
    { title: 'Tên loại nhà', index: 'Name' },
  ...
  addOrUpdate(record?: any): void {
    const drawerRef = this.drawerService.create<AddorupdateMd167HouseTypeComponent>({
      nzTitle: record ? `Sửa loại nhà: "${record.Name}"` : 'Thêm mới loại nhà',
      // record.khoa_chinh
      nzWidth: '80vw',
      nzContent: AddorupdateMd167HouseTypeComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record
      }
