...
import { Md167HouseFileRepository } from 'src/app/infrastructure/repositories/Md167HouseFile.repository';
...
@Component({
  selector: 'app-md167-house-file',
  templateUrl: './md167-house-file.component.html',
  styles: [
  ]
})
export class Md167HouseFileComponent implements OnInit {
  ...
  constructor(
    private md167HouseFileRepository: Md167HouseFileRepository,
    private houseRepository: Md167HouseRepository,
  ...
  ngOnInit(): void {
    this.validateForm = this.fb.group({
      ...
      Md167HouseId: [this.record ? this.record.Id : undefined],
  ...
  async submitForm() {
    ...
    const resp = await this.md167HouseFileRepository.addNew(data);
  ...
  async getData() {
    ...
    this.paging.query = `Md167HouseId=${this.record.Id}`;
    this.paging.order_by = 'CreatedAt Desc';
    try {
      this.loading = true;
      const resp = await this.md167HouseFileRepository.getByPage(this.paging);
  ...
  async delete(data: any) {
    const resp = await this.md167HouseFileRepository.delete(data);
    if (resp.meta?.error_code == 200) {
