...
@Component({
  selector: 'app-extra-support-requests',
  templateUrl: './extra-support-requests.component.html',
  styles: [
  ]
})
export class ExtraSupportRequestsComponent implements OnInit {
  data: any;
  event: any;
  listData: any;
  loading = false;
  ...
  constructor(
    ...
    private extraSupportRequestRepository: ExtraSupportRequestRepository,
  ) {
    ...
    this.columns =[
      ...
      { title: 'Trạng thái yêu cầu hỗ trợ', index: 'TypeSupportReq' , className: 'text-center', type: 'enum', enum: TypeSupportReq},
      {
          title: 'Chức năng',
          width: 100,
          ...
          buttons: [
            ...
            },
            {
              ...
              type: 'del',
              pop: {
                title: 'Bạn có chắc chắn xóa yêu cầu hỗ trợ này?',
                okType: 'danger',
                icon: 'star'
              },
  ...
  addOrUpdate(record?: any): void {
    ...
    let typereq_data = [...this.typereq_data];

    const drawerRef = this.drawerService.create<AddOrUpdateComponent>({
      nzTitle: record ? `Sửa yêu cầu hỗ trợ` : 'Thêm mới yêu cầu hỗ trợ',
      nzWidth: '55vw',
    ...
    drawerRef.afterClose.subscribe((data: any) => {
      if (data) {
        let msg = data.Id ? `Sửa yêu cầu hỗ trợ thành công!` : `Thêm mới yêu cầu hỗ trợ ${data.Code} thành công!`;
  ...
  async delete(data: any) {
    const resp = await this.extraSupportRequestRepository.delete(data);
  ...
  try {
      this.loading = true;
      const resp = await this.extraSupportRequestRepository.getByPage(this.paging);
