...
import { AddOrUpdatePlatformReportComponent } from './add-or-update/add-or-update.component';
...
@Component({
  selector: 'app-tdc-report-platform',
  templateUrl: './tdc-report-platform.component.html',
  styles: [
  ]
})
export class TdcReportPlatformComponent implements OnInit {
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.reportPlatformRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    ...
    const drawerRef = this.drawerService.create<AddOrUpdatePlatformReportComponent>({
      nzTitle: record ? `Sửa căn hộ` : 'Thêm mới nền đất',
      // record.khoa_chinh
      nzWidth: '75vw',
      nzContent: AddOrUpdatePlatformReportComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
    ...
    drawerRef.afterClose.subscribe((data: any) => {
      if (data) {
        let msg = data.Id ? `Sửa danh sách nền đất thành công!` : `Thêm mới nền đất thành công!`;
  ...
  async delete(data: any) {
    const resp = await this.reportPlatformRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  async csvExport() {
    const resp = await this.reportPlatformRepository.ExportExcel(this.data);
  }

  async csvExport2() {
    const resp = await this.reportPlatformRepository.ExcelRP2(this.selectedTypeId);
  }

  async csvExport3() {
    const resp = await this.reportPlatformRepository.ExcelRP3();
  }

  async csvExport4() {
    const resp = await this.reportPlatformRepository.ExcelRP4();
  }
