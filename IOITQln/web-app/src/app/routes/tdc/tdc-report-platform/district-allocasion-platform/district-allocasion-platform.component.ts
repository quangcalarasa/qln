...
@Component({
  selector: 'app-district-allocasion-platform',
  templateUrl: './district-allocasion-platform.component.html',
  styles: [
  ]
})
export class DistrictAllocasionPlatformComponent implements OnInit {
  @ViewChild('tableItemRef') private tableItemRef!: STComponent;
  @Input() districtAllocasionPlatform: any[] = [];
  ...
  district_allocasion_platform_data: any[] = [];
  ...
  ngOnInit(): void {
    this.data_tableItemRef = [...this.districtAllocasionPlatform];
    this.invalid_tableItemRef = this.districtAllocasionPlatform.length == 0 ? true : false;
  ...
  async getDistrictAllocasionData() {
    ...
    if (resp.meta?.error_code == 200) {
      this.district_allocasion_platform_data = resp.data;
      this.district_allocasion_platform_data.forEach((value: any) => {
        value.disabled = false;
      });
      if (this.district_allocasion_platform_data) localStorage.setItem('data', JSON.stringify(this.district_allocasion_platform_data));
    }
  }
  ...
  uploadDistrictAllocasionData() {
    ...
    this.district_allocasion_platform_data.forEach((value, index) => {
      datatable.forEach((val, i) => {
        if (value.Id == val.DistrictId) this.district_allocasion_platform_data[index].disabled = false;
      });
    });
    this.district_allocasion_platform_data = [...this.district_allocasion_platform_data];
  }

  onSelect(index: number, value: string) {
    this.selectedValue = value;
    const selectedOption = this.district_allocasion_platform_data.find(option => option.Id === value);
    if (selectedOption) {
