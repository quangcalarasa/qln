...
import { AddOrUpdateLandComponent } from './add-or-update-land/add-or-update-land.component';
...
export class LandComponent implements OnInit {
  ...
  constructor(
    ...
    this.columns = [
      ...
      { title: 'Mã lô', index: 'Code' },
      { title: 'Tên lô', render: 'LandName' },
      { title: 'Dự án', index: 'TDCProjectName'},
      { title: 'Số lượng khối', index:'BlockHouseCount'},
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.landRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    let add;
    let project_tdc_data = this.project_tdc_data;
    record ? add = true : add = false
    localStorage.setItem('add', add.toString())
    const drawerRef = this.drawerService.create<AddOrUpdateLandComponent>({
      nzTitle: record ? `Sửa ${this.titleString} : ${record.Code}` : `Thêm mới ${this.titleString}`,
      // record.khoa_chinh
      nzWidth: '60vw',
      nzContent: AddOrUpdateLandComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
        project_tdc_data,
  ...
  async delete(data: any) {
    const resp = await this.landRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.TdcLand
        }
    });
  }

  async csvExport() {
    const resp = await this.landRepository.ExportExcel();
  }
