...
@Component({
  selector: 'app-tdc-auth-customer',
  templateUrl: './tdc-auth-customer.component.html',
  styles: []
})
export class TdcAuthCustomerComponent implements OnInit {
  @ViewChild('tableItemRef') private tableItemRef!: STComponent;
  @Input() tdcAuthCustomerDetailDatas: any[] = [];
  ...
  ngOnInit(): void {
    this.tdcAuthCustomerDetailDatas.map((item: any) => {
      (item.PdwTTC = [item.ProvinceTt, item.DistrictTt, item.WardTt]), (item.PdwLHC = [item.ProvinceLh, item.DistrictLh, item.WardLh]);
      return item;
    });
    this.data_tableItemRef = [...this.tdcAuthCustomerDetailDatas];
    this.invalid_tableItemRef = this.tdcAuthCustomerDetailDatas.length == 0 ? true : false;
    this.getAllLaneData();
  }
  ...
  private submit(i: STData): void {
    if (!i['FullName'] || !i['CCCD'] || !i['Dob'] || !i['Phone'] || !i['AddressTt'] || !i['AddressLh']) {
