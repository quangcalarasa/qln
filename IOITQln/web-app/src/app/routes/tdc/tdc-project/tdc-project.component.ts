...
import { AddOrUpdateProjectComponent } from './add-or-update/add-or-update-project.component';
...
@Component({
  selector: 'app-tdc-project',
  templateUrl: './tdc-project.component.html',
  styles: [
  ]
})
export class TdcProjectComponent implements OnInit {
  ...
  titleString?: string = 'dự án tái định cư';
  ...
  constructor(
    ...
    this.columns = [
      ...
      { title: 'Mã dự án', index: 'Code' },
      { title: 'Tên dự án', index: 'Name' },
      { title: 'Số lượng lô', index: 'LandCount' },
      { title: 'Địa chỉ', index: 'FullAddress' },
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.tDCProjectRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    ...
    const drawerRef = this.drawerService.create<AddOrUpdateProjectComponent>({
      nzTitle: record ? `Sửa ${this.titleString} : ${record.Code}` : `Thêm mới ${this.titleString}`,
      // record.khoa_chinh
      nzWidth: '60vw',
      nzContent: AddOrUpdateProjectComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
      }
  ...
  async delete(data: any) {
    const resp = await this.tDCProjectRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  async csvExport() {
    const resp = await this.tDCProjectRepository.ExportExcel();
  }
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.TdcProject
        }
    });
  }
}
