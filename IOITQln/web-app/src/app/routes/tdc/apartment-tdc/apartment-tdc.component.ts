...
import { AddOrUpdateApartmentTdcComponent } from './add-or-update-apartmenttdc/add-or-update-apartmenttdc.component';
...
@Component({
  selector: 'app-apartment-tdc',
  templateUrl: './apartment-tdc.component.html',
  styles: [
  ]
})
export class ApartmentTdcComponent implements OnInit {
  ...
  constructor(
    ...
    this.columns = [
      ...
      { title: 'Mã căn', index: 'Code' },
      { title: 'Tên căn', index: 'Name' },
      { title: 'Tầng', index: 'FloorTdcName'},
  ...
  async getData() {
    ...
    if (this.query.type != undefined) {
      this.paging.query += ` and FloorTdcId=${this.query.type}`
    }
    ...
    try {
      this.loading = true;
      const resp = await this.apartmentTdcRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    let add;
    let floor_tdc_data = this.floor_tdc_data;
    record ? add = true : add = false
    localStorage.setItem('add', add.toString())
    const drawerRef = this.drawerService.create<AddOrUpdateApartmentTdcComponent>({
      nzTitle: record ? `Sửa ${this.titleString} : ${record.Code}` : `Thêm mới ${this.titleString}`,
      // record.khoa_chinh
      nzWidth: '60vw',
      nzContent: AddOrUpdateApartmentTdcComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
        floor_tdc_data
  ...
  async delete(data: any) {
    const resp = await this.apartmentTdcRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  findFloorTdc(id:number){
    let item = this.floor_tdc_data.find(x => x.Id == id);
    return  item ? item.Name : undefined;
  }
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.TdcApartment
        }
    });
  }

  async csvExport() {
    const resp = await this.apartmentTdcRepository.ExportExcel();
  }
}
