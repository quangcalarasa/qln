...
import { AddOrUpdateOPATComponent } from './add-or-update/add-or-update-OPAT.component';
...
@Component({
  selector: 'app-original-price-and-tax',
  templateUrl: './original-price-and-tax.component.html',
  styles: []
})
export class OriginalPriceAndTaxComponent implements OnInit {
  ...
  titleString?: string = 'Thành phần giá gốc thuế - phí';
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.originalPriceAndTaxRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    ...
    const drawerRef = this.drawerService.create<AddOrUpdateOPATComponent>({
      nzTitle: record ? `Sửa ${this.titleString} : ${record.Code}` : `Thêm mới ${this.titleString}`,
      // record.khoa_chinh
      nzWidth: '60vw',
      nzContent: AddOrUpdateOPATComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record
      }
  ...
  async delete(data: any) {
    const resp = await this.originalPriceAndTaxRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  async csvExport() {
    const resp = await this.originalPriceAndTaxRepository.ExportExcel();
  }
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.TdcOriginalPriceAndTax
        }
    });
  }
