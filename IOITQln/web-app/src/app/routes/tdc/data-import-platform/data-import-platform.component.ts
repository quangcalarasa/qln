...
import { DataImportPlatformRepository } from 'src/app/infrastructure/repositories/data-import-platform.repository';
@Component({
  selector: 'app-data-import-platform',
  templateUrl: './data-import-platform.component.html',
  styles: [
  ]
})
export class DataImportPlatformComponent implements OnInit {
  ...
  constructor(private uploadRepository: UploadRepository, 
    private importHistoryRepository: ImportHistoryRepository, private dataImportPlatformRepository: DataImportPlatformRepository) { }
  ...
  async getData() {
    ...
    const resp = await this.importHistoryRepository.getByPage(this.paging, ImportHistoryTypeEnum.Tdc_PlatformDataImport);
  ...
  async handleChange(file: any) {
    ...
    this.loading = true;
    const resp_ren = await this.dataImportPlatformRepository.importDataExcel(formData);
  ...
  downloadTemplate() {
    this.uploadRepository.downloadFileExcelTemplate(ImportHistoryTypeEnum.Tdc_PlatformDataImport);
  }
