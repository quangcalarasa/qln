...
import { AddOrUpdateProfitValueComponent } from './add-or-update/add-or-update.component';
...
export class ProfitValueComponent implements OnInit {
  ...
  async getData() {
    ...
    try {
      this.loading = true;
      const resp = await this.profitValueRepository.getByPage(this.paging);
  ...
  addOrUpdate(record?: any): void {
    ...
    const drawerRef = this.drawerService.create<AddOrUpdateProfitValueComponent>({
      nzTitle: record ? `Sửa ${this.titleString} : ${record.Id}` : `Thêm mới ${this.titleString}`,
      // record.khoa_chinh
      nzWidth: '60vw',
      nzContent: AddOrUpdateProfitValueComponent,
      nzPlacement: 'left',
      nzContentParams: {
        record,
  ...
  async delete(data: any) {
    const resp = await this.profitValueRepository.delete(data);
    if (resp.meta?.error_code == 200) {
  ...
  async csvExport() {
    const resp = await this.profitValueRepository.ExportExcel();
  }
  ...
  import() {
    this.drawerService.create<SharedImportExcelComponent>({
        ...
        nzContentParams: {
            importHistoryType: ImportHistoryTypeEnum.TdcProfitValue
        }
    });
  }
}
