...
import { AddOrUpdateDistributionFloorCoefficientComponent } from './add-or-update/add-or-update-distribution-floor-coefficient.component';
...
@Component({
    selector: 'app-cate-distribution-floor-coefficient',
    templateUrl: './distribution-floor-coefficient.component.html'
})
export class DistributionFloorCoefficientComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        // { title: 'Áp dụng hệ số tầng lửng', index: 'ApplyMezzanineCoefficient', type: 'yn', safeType: "safeHtml", className: 'text-center' },
        { title: 'Hệ số tầng lửng', render: 'mcClmn', className: 'text-center' },
        { title: 'Hệ số nhà chung cư', render: 'fcClmn', className: 'text-center' },
        {
            title: 'Chức năng',
            width: 100,
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá Hệ số phân bổ các tầng này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        try {
            this.loading = true;
            const resp = await this.distributionFloorCoefficientRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdateDistributionFloorCoefficientComponent>({
            nzTitle: record ? `Sửa thông tin Hệ số phân bổ các tầng` : 'Thêm mới thông tin Hệ số phân bổ các tầng',
            // record.khoa_chinh
            nzWidth: '75vw',
            nzContent: AddOrUpdateDistributionFloorCoefficientComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa thông tin Hệ số phân bổ các tầng thành công!` : `Thêm mới thông tin Hệ số phân bổ các tầng thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.distributionFloorCoefficientRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa thông tin Hệ số phân bổ các tầng thành công!`);
            this.getData();
        } else {
