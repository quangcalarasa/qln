...
import { AddOrUpdatePositionCoefficientComponent } from './add-or-update/add-or-update-position-coefficient.component';
...
@Component({
    selector: 'app-cate-position-coefficient',
    templateUrl: './position-coefficient.component.html'
})
export class PositionCoefficientComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        { title: 'Áp dụng theo', index: 'Name' },
        { title: 'Ngày áp dụng', index: 'DoApply', type: 'date', className: 'text-center', width: 150, dateFormat: 'dd/MM/yyyy' },
        { title: 'Vị trí 1', render: 'value1Clmn', className: 'text-center' },
        { title: 'Vị trí 2', render: 'value2Clmn', className: 'text-center' },
        { title: 'Vị trí 3', render: 'value3Clmn', className: 'text-center' },
        { title: 'Vị trí 4', render: 'value4Clmn', className: 'text-center' },
        {
            title: 'Chức năng',
            width: 100,
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá thông tin Hệ số vị trí này này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        try {
            this.loading = true;
            const resp = await this.positionCoefficientRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdatePositionCoefficientComponent>({
            nzTitle: record ? `Sửa thông tin Hệ số vị trí` : 'Thêm mới thông tin Hệ số vị trí',
            // record.khoa_chinh
            nzWidth: '75vw',
            nzContent: AddOrUpdatePositionCoefficientComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa thông tin Hệ số vị trí thành công!` : `Thêm mới thông tin Hệ số vị trí thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.positionCoefficientRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa thông tin Hệ số vị trí thành công!`);
            this.getData();
        } else {
