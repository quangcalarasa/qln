...
import { AddOrUpdateDeductionCoefficientComponent } from './add-or-update/add-or-update-deduction-coefficient.component';
...
@Component({
    selector: 'app-cate-deduction-coefficient',
    templateUrl: './deduction-coefficient.component.html'
})
export class DeductionCoefficientComponent implements OnInit {
    ...
    // decree_type1_data = [];
    decree_type2_data = [];
    role = this.commonService.CheckAccessKeyRole(AccessKey.DEDUCTION_COEFFICIENT_MANAGEMENT);
    columns: STColumn[] = [
        ...
        { title: 'Đối tượng miễn giảm', index: 'ObjectApply' },
        { title: 'Hệ số được miễn giảm', render: 'vClmn', className: 'text-center' },
        { title: 'Diễn giải', index: 'Note' },
        {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá hệ số được miễn giảm tiền nhà này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        if (this.query.txtSearch != undefined && this.query.txtSearch != '') {
            if (this.query.txtSearch.trim() != '')
                this.paging.query += ` and (ObjectApply.Contains("${this.query.txtSearch}") OR Note.Contains("${this.query.txtSearch}"))`;
        }

        try {
            this.loading = true;
            const resp = await this.deductionCoefficientRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdateDeductionCoefficientComponent>({
            nzTitle: record ? `Sửa thông tin hệ số được miễn giảm tiền nhà` : 'Thêm mới thôn tin hệ số được miễn giảm tiền nhà',
            // record.khoa_chinh
            nzWidth: '60vw',
            nzContent: AddOrUpdateDeductionCoefficientComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa thông tin hệ số được miễn giảm tiền nhà thành công!` : `Thêm mới thông tin hệ số được miễn giảm tiền nhà thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.deductionCoefficientRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa thông tin hệ số được miễn giảm tiền nhà thành công!`);
            this.getData();
        } else {
