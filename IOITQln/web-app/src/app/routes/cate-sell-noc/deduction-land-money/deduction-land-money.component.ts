...
import { AddOrUpdateDeductionLandMoneyComponent } from './add-or-update/add-or-update-deduction-land-money.component';
...
@Component({
    selector: 'app-cate-deduction-land-money',
    templateUrl: './deduction-land-money.component.html'
})
export class DeductionLandMoneyComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        { title: 'Điều kiện miễn giảm', index: 'Condition' },
        ...
        { title: 'Giá trị (%)', render: 'valueClmn', className: 'text-center' },
        {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá thông tin Tiền đất miễn giảm này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        if (this.query.txtSearch != undefined && this.query.txtSearch != '') {
            if (this.query.txtSearch.trim() != '')
                this.paging.query += ` and Condition.Contains("${this.query.txtSearch}")`;
        }

        try {
            this.loading = true;
            const resp = await this.deductionLandMoneyRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdateDeductionLandMoneyComponent>({
            nzTitle: record ? `Sửa thông tin Tiền đất miễn giảm` : 'Thêm mới thông tin Tiền đất miễn giảm',
            // record.khoa_chinh
            nzWidth: '75vw',
            nzContent: AddOrUpdateDeductionLandMoneyComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa thông tin Tiền đất miễn giảm thành công!` : `Thêm mới thông tin Tiền đất miễn giảm thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.deductionLandMoneyRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa thông tin Tiền đất miễn giảm thành công!`);
            this.getData();
        } else {
