...
import { AddOrUpdatePricelistComponent } from './add-or-update/add-or-update-price-list.component';
...
export class PriceListComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        // { title: 'Tên loại công trình', index: 'NameOfConstruction' },
        // { title: 'Là lửng', index: 'IsMezzanine', type: 'yn', safeType: "safeHtml", className: "text-center" },
        ...
        { title: 'Căn cứ', index: 'Des' },
        {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá bảng giá nhà ở này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        try {
            this.loading = true;
            const resp = await this.priceListRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdatePricelistComponent>({
            nzTitle: record ? `Sửa bảng giá nhà ở` : 'Thêm mới bảng giá nhà ở',
            // record.khoa_chinh
            nzWidth: '75vw',
            nzContent: AddOrUpdatePricelistComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa bảng giá nhà ở thành công!` : `Thêm mới bảng giá nhà ở thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.priceListRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa bảng giá nhà ở thành công!`);
            this.getData();
        } else {
