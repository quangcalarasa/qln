...
import { AddOrUpdateAreaCorrectionCoefficientComponent } from './add-or-update/add-or-update-area-correction-coefficient.component';
...
@Component({
    selector: 'app-cate-area-correction-coefficient',
    templateUrl: './area-correction-coefficient.component.html'
})
export class AreaCorrectionCoefficientComponent implements OnInit {
    ...
    columns: STColumn[] = [
        ...
        { title: 'Vùng quy định', index: 'Des' },
        { title: 'Chi tiết vùng quy định', index: 'Name' },
        { title: 'Hệ số điều chỉnh vùng', render: 'valueClmn', width: 150 },
        { title: 'Ngày áp dụng', index: 'DoApply', type: 'date', className: 'text-center', width: 150, dateFormat: 'dd/MM/yyyy' },
        {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    pop: {
                        title: 'Bạn có chắc chắn muốn xoá Hệ số điều chỉnh vùng này?',
                        okType: 'danger',
                        icon: 'star'
                    },
    ...
    async getData() {
        ...
        if (this.query.txtSearch != undefined && this.query.txtSearch != '') {
            if (this.query.txtSearch.trim() != '')
                this.paging.query += ` and (Des.Contains("${this.query.txtSearch}") OR Name.Contains("${this.query.txtSearch}"))`;
        }

        try {
            this.loading = true;
            const resp = await this.areaCorrectionCoefficientRepository.getByPage(this.paging);
    ...
    addOrUpdate(record?: any): void {
        const drawerRef = this.drawerService.create<AddOrUpdateAreaCorrectionCoefficientComponent>({
            nzTitle: record ? `Sửa thông tin Hệ số điều chỉnh vùng` : 'Thêm mới thông tin Hệ số điều chỉnh vùng',
            // record.khoa_chinh
            nzWidth: '60vw',
            nzContent: AddOrUpdateAreaCorrectionCoefficientComponent,
            nzPlacement: 'left',
            nzContentParams: {
                record,
        ...
        drawerRef.afterClose.subscribe((data: any) => {
            if (data) {
                let msg = data.Id ? `Sửa thông tin Hệ số điều chỉnh vùng thành công!` : `Thêm mới thông tin Hệ số điều chỉnh vùng thành công!`;
    ...
    async delete(data: any) {
        const resp = await this.areaCorrectionCoefficientRepository.delete(data);
        if (resp.meta?.error_code == 200) {
            this.message.create('success', `Xóa thông tin Hệ số điều chỉnh vùng thành công!`);
            this.getData();
        } else {
