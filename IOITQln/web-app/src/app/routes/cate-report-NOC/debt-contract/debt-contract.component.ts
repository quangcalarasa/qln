...
@Component({
  selector: 'app-report-noc-debt-contract',
  templateUrl: './debt-contract.component.html',
  styles: []
})
export class DebtContractReportComponent implements OnInit {
  ...
  public statusBar: {
    ...
    public columnDefs: (ColDef | ColGroupDef)[] = [
      ...
      { headerName: 'Ngày ký hợp đồng', field: 'DateAssgint', valueFormatter: (data: any) => {
      ...
      {
        headerName: 'Số tiền phải thanh toán', field: 'Total', valueFormatter: (data: any) => {
          return data.value ? data.value.toLocaleString("de-DE", { maximumFractionDigits: 0, minimumFractionDigits: 0 }) : "";
        }, onCellClicked: (evt) => { this.viewRow(evt) }
      },
      {
        headerName: 'Số tiền đã thanh toán', field: 'Paid', valueFormatter: (data: any) => {
          return data.value ? data.value.toLocaleString("de-DE", { maximumFractionDigits: 0, minimumFractionDigits: 0 }) : "";
        }, onCellClicked: (evt) => { this.viewRow(evt) }
      },
      {
        headerName: 'Còn nợ', field: 'Diff', valueFormatter: (data: any) => {
          return data.value ? data.value.toLocaleString("de-DE", { maximumFractionDigits: 0, minimumFractionDigits: 0 }) : "";
        }, onCellClicked: (evt) => { this.viewRow(evt) }
      },
  ...
  async getDataReport() {
    ...
    const resp = await this.reportNocRepository.GetDataExport(this.validateForm.value);
