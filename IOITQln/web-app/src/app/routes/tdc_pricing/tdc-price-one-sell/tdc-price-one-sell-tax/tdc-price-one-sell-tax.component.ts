...
@Component({
  selector: 'app-tdc-price-one-sell-tax',
  templateUrl: './tdc-price-one-sell-tax.component.html',
  styles: [
  ]
})
export class TdcPriceOneSellTaxComponent implements OnInit {
  @ViewChild('tableItemRef') private tableItemRef!: STComponent;
  @Input() tdcPriceOneSellTaxes: any[] = [];
  ...
  columnsItem: STColumn[] = [
    ...
    { renderTitle: 'TotalHeader', render: 'totalClmn' },
    {
      title: 'Chức năng',
      width: 150,
  ...
  ngOnInit(): void {
    if (this.tdcPriceOneSellTaxes == undefined) {
      this.tdcPriceOneSellTaxes = [];
    }
    this.data_tableItemRef = [...this.tdcPriceOneSellTaxes];

    this.invalid_tableItemRef = this.tdcPriceOneSellTaxes.length == 0 ? true : false;
  }
