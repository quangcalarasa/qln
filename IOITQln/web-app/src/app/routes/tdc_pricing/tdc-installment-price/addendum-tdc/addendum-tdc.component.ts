...
import { TypeContractTDC } from 'src/app/shared/utils/enums';

@Component({
  selector: 'app-addendum-tdc',
  templateUrl: './addendum-tdc.component.html',
  styles: []
})
export class AddendumTDCComponent implements OnInit {
  @Input() parent: any;
  @Input() record: any;
  ...
  ngOnInit(): void {
    ...
    this.validateForm = this.fb.group({
      ...
      ContractNumber: [this.record ? this.record.ContractNumber : this.parent.ContractNumber], //số hợp đồng
      DateNumber: [this.record ? convertDate(this.record.DateNumber) : convertDate(this.parent.DateNumber), [Validators.required]], //ngày hợp đồng
      TdcCustomerId: [this.record ? this.record.TdcCustomerId : this.parent.TdcCustomerId], //id khách hàng
      ...
      Type: [this.record ? this.record.Type : TypeContractTDC.EXTRA], //Loại hợp đồng: hợp đồng hoặc phụ lục
  ...
  changeDataOfficalArea(event: any) {
    ...
    // this.validateForm.get('tDCInstallmentTemporaryDetails')?.setValue(event);
    this.TotalArea = area;
