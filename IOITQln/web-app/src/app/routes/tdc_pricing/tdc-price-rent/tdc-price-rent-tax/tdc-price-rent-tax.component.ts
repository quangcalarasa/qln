...
@Component({
  selector: 'app-tdc-price-rent-tax',
  templateUrl: './tdc-price-rent-tax.component.html'
})
export class TdcPriceRentTaxComponent implements OnInit {
  @ViewChild('tableItemRef') private tableItemRef!: STComponent;
  @Input() tdcPriceRentTaxes: any[] = [];
  ...
  ngOnInit(): void {
    if (this.tdcPriceRentTaxes == undefined) {
      this.tdcPriceRentTaxes = [];
    }
    this.data_tableItemRef = [...this.tdcPriceRentTaxes];

    this.invalid_tableItemRef = this.tdcPriceRentTaxes.length == 0 ? true : false;
  }
