...
namespace IOITQln.Controllers.ApiTdc
{
    ...
    [ApiController]
    public class OriginalPriceAndTaxController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("priceandtax", "priceandtax");
        private readonly ApiDbContext _context;
        string functionCode = "ORIGINAL_PRICE_AND_TAX";
        private IMapper _mapper;
        private IHostingEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;
        public OriginalPriceAndTaxController(ApiDbContext context, IMapper mapper, IHostingEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<OriginalPriceAndTax> data = _context.OriginalPriceAndTaxs.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                OriginalPriceAndTax data = await _context.OriginalPriceAndTaxs.FindAsync(id);
        ...
        // POST: api/OriginalPriceAndTax
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] OriginalPriceAndTax input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (OriginalPriceAndTax)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.OriginalPriceAndTaxs.Add(input);
                    ...
                    try
                    {
                        ...
                        string code = "";
                        int newid = input.Id;
                        if (newid < 10) code = "GGTP000" + newid;
                        if (newid >= 10) code = "GGTP00" + newid;
                        if (newid >= 100) code = "GGTP0" + newid;
                        if (newid >= 1000) code = "GGTP" + newid;
                        input.Code = code;
                        OriginalPriceAndTax originalPriceAndTax = _context.OriginalPriceAndTaxs.Find(newid);
                        originalPriceAndTax.Code = code;
                        _context.Update(originalPriceAndTax);
                        await _context.SaveChangesAsync();
                        //thêm LogAction

                        LogActionModel logActionModel = new LogActionModel("Thêm mới thành phần giá gốc - thuế phí: " + input.Name, "OriginalPriceAndTax", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (OriginalPriceAndTaxExists(input.Id))
                        {
        ...
        // PUT: api/OriginalPriceAndTax/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] OriginalPriceAndTax input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                OriginalPriceAndTax data = await _context.OriginalPriceAndTaxs.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                OriginalPriceAndTax codeExist = _context.OriginalPriceAndTaxs.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thành phần giá bán cấu thành: " + input.Name, "OriginalPriceAndTax", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!OriginalPriceAndTaxExists(data.Id))
                        {
        ...
        // DELETE: api/OriginalPriceAndTax/1
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //TDCProjectPriceAndTax
                TDCProjectPriceAndTax itemExist = _context.TDCProjectPriceAndTaxs.Where(b => b.PriceAndTaxId == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (itemExist != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (TDCProjectPriceAndTax). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thành phần giá bán cấu thành: " + data.Name, "OriginalPriceAndTax", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool OriginalPriceAndTaxExists(int id)
        {
            return _context.OriginalPriceAndTaxs.Count(e => e.Id == id) > 0;
        }
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<OriginalPriceAndTax> originalPriceAndTaxes = _context.OriginalPriceAndTaxs.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"templates/OriginalPriceAndTax.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
            string templatePath = Path.Combine(webRootPath, template);

            MemoryStream ms = WriteDataToExcel(templatePath, 0, originalPriceAndTaxes);
            byte[] byteArrayContent = ms.ToArray();
            return new FileContentResult(byteArrayContent, "application/octet-stream");
        }

        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<OriginalPriceAndTax> data)
        {
        ...
        //public async Task<IActionResult> ImportExcel([FromBody] List<OriginalPriceAndTax> input)
        //{
        //    string accessToken = Request.Headers[HeaderNames.Authorization];
        //    Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
        //    if (token == null)
        ...
        //                {
        //                    var existCode = _context.OriginalPriceAndTaxs.FirstOrDefault(x => x.Code == item.Code);
        //                    if (existCode != null)
        //                    {
        ...
        //                    _context.OriginalPriceAndTaxs.Add(item);

        //                    await _context.SaveChangesAsync();

        //                    LogActionModel logActionModel = new LogActionModel("Thêm mới thành phần giá gốc thuế phí: " + item.Name, "OriginalPriceAndTax", item.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        //                    LogAction logAction = _mapper.Map<LogAction>(logActionModel);
        //                    _context.Add(logAction);
        //                    await _context.SaveChangesAsync();
        ...
        //                    def.data = item;
        //                    def.data = messages;
        //                    messages.Add("Import thành công");
        //                }
        //                transaction.Commit();
        //                return Ok(def);
        //            }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.TdcOriginalPriceAndTax;

                List<OriginalPriceAndTaxDataImport> data = new List<OriginalPriceAndTaxDataImport>();
                ...
                List<OriginalPriceAndTaxDataImport> dataValid = data.Where(e => e.Valid == true).ToList();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var dataItem in data)
                        {
                            if (dataItem.Valid == true)
                            {
                                ...
                                dataItem.Code = "GGTP";
                                dataItem.Code = CodeIndentity.CodeInd("GGTP", _context.OriginalPriceAndTaxs.Count(), 4);
                        ...
                        _context.OriginalPriceAndTaxs.AddRange(dataValid);
        ...
        public static List<OriginalPriceAndTaxDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<OriginalPriceAndTaxDataImport> res = new List<OriginalPriceAndTaxDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    OriginalPriceAndTaxDataImport input1Detai = new OriginalPriceAndTaxDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 1)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột tên thành phần chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột tên thành phần\n";
                                }
                            }
                            else if (i == 2)
                            {
