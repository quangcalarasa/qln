...
namespace IOITQln.Controllers.ApiTdc
{
    ...
    [ApiController]
    public class IngredientsPriceController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ingreprice", "ingreprice");
        private readonly ApiDbContext _context;
        string functionCode = "INGREDIENTS_PRICE";
        private IMapper _mapper;
        private IHostingEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;
        public IngredientsPriceController(ApiDbContext context, IMapper mapper, IHostingEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<IngredientsPrice> data = _context.IngredientsPrices.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById( int id)
        {
            ...
            try
            {
                IngredientsPrice data = await _context.IngredientsPrices.FindAsync(id);
        ...
        // POST: api/IngredientsPrice
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] IngredientsPrice input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (IngredientsPrice)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.IngredientsPrices.Add(input);
                    ...
                    try
                    {
                        ...
                        string code = "";
                        int newid = input.Id;
                        if (newid < 10) code = "GBCT000" + newid;
                        if (newid >= 10) code = "GBCT00" + newid;
                        if (newid >= 100) code = "GBCT0" + newid;
                        if (newid >= 1000) code = "GBCT" + newid;
                        input.Code = code;
                        IngredientsPrice ingredientsPrice = _context.IngredientsPrices.Find(newid);
                        ingredientsPrice.Code = code;
                        _context.Update(ingredientsPrice);
                        await _context.SaveChangesAsync();
                        //thêm LogAction

                        LogActionModel logActionModel = new LogActionModel("Thêm mới thành phần giá bán cấu thành: " + input.Name, "IngredientsPrice", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (IngredientsPriceExists(input.Id))
                        {
        ...
        // PUT: api/IngredientsPrice/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put( int id, [FromBody] IngredientsPrice input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                IngredientsPrice data = await _context.IngredientsPrices.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                IngredientsPrice codeExist = _context.IngredientsPrices.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thành phần giá bán cấu thành: " + input.Name, "IngredientsPrice", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!IngredientsPriceExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //TDCProjectIngrePrice
                TDCProjectIngrePrice itemExist = _context.TDCProjectIngrePrices.Where(b => b.IngredientsPriceId == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (itemExist != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (TDCProjectIngrePrice). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thành phần giá bán cấu thành: " + data.Name, "IngredientsPrice", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool IngredientsPriceExists(int id)
        {
            return _context.IngredientsPrices.Count(e => e.Id == id) > 0;
        }
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<IngredientsPrice> ingredientsPrices = _context.IngredientsPrices.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            MemoryStream ms = WriteDataToExcel(templatePath, 0, ingredientsPrices);
            byte[] byteArrayContent = ms.ToArray();
            return new FileContentResult(byteArrayContent, "application/octet-stream");
        }

        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<IngredientsPrice> data)
        {
        ...
        //public async Task<IActionResult> ImportExcel([FromBody] List<IngredientsPrice> input)
        //{
        //    string accessToken = Request.Headers[HeaderNames.Authorization];
        //    Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
        //    if (token == null)
        ...
        //                {
        //                    var existCode = _context.IngredientsPrices.FirstOrDefault(x => x.Code == item.Code);
        //                    if (existCode != null)
        //                    {
        ...
        //                    _context.IngredientsPrices.Add(item);

        //                    await _context.SaveChangesAsync();

        //                    LogActionModel logActionModel = new LogActionModel("Thêm mới thành phần giá bán cấu thành: " + item.Name, "IngredientsPrice", item.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        //                    LogAction logAction = _mapper.Map<LogAction>(logActionModel);
        //                    _context.Add(logAction);
        //                    await _context.SaveChangesAsync();
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.TdcIngredientsPrice;

                List<IngredientsPriceDataImport> data = new List<IngredientsPriceDataImport>();
                ...
                List<IngredientsPriceDataImport> dataValid = data.Where(e => e.Valid == true).ToList();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var dataItem in data)
                        {
                            if (dataItem.Valid == true)
                            {
                                ...
                                dataItem.Code = "GBCT";
                                dataItem.Code = CodeIndentity.CodeInd("GBCT", _context.IngredientsPrices.Count(), 4);
                        ...
                        _context.IngredientsPrices.AddRange(dataValid);
        ...
        public static List<IngredientsPriceDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<IngredientsPriceDataImport> res = new List<IngredientsPriceDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    IngredientsPriceDataImport input1Detai = new IngredientsPriceDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 1)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột tên thành phần giá cấu thành chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột tên thành phần giá cấu thành\n";
                                }
                            }
                            else if (i == 2)
                            {
