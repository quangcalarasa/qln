...
namespace IOITQln.Controllers.ApiTdc
{
    ...
    [ApiController]
    public class AnnualInstallmentController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("annual", "annual");
        private readonly ApiDbContext _context;
        string functionCode = "ANNUAL_INSTALLMENT_INTEREST_RATE";
        private IMapper _mapper;
        private IHostingEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;
        public AnnualInstallmentController(ApiDbContext context, IMapper mapper, IHostingEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<AnnualInstallment> data = _context.AnnualInstallments.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<AnnualInstallmentData> res = _mapper.Map<List<AnnualInstallmentData>>(data.ToList());
                        foreach (AnnualInstallmentData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                AnnualInstallment data = await _context.AnnualInstallments.FindAsync(id);
        ...
        // POST: api/AnnualInstallment
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AnnualInstallment input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (AnnualInstallment)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.AnnualInstallments.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới lãi suất trả góp theo năm: " + input.Id, "AnnualInstallment", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (AnnualInstallmentExists(input.Id))
                        {
        ...
        // PUT: api/AnnualInstallment/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AnnualInstallment input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                AnnualInstallment data = await _context.AnnualInstallments.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa lãi suất trả góp theo năm: " + input.Id, "AnnualInstallment", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!AnnualInstallmentExists(data.Id))
                        {
        ...
        // DELETE: api/AnnualInstallment/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thành phần giá bán cấu thành: " + data.Id, "AnnualInstallment", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool AnnualInstallmentExists(int id)
        {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<AnnualInstallment> annualInstallments = _context.AnnualInstallments.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<AnnualInstallmentData> mapper_dataTdc = _mapper.Map<List<AnnualInstallmentData>>(annualInstallments);
            ...
            string template = @"templates/AnnualInstallment.xlsx";
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<AnnualInstallmentData> data)
        {
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.TdcInstallmentRate;

                List<AnnualInstallmentDataImport> data = new List<AnnualInstallmentDataImport>();
                ...
                List<AnnualInstallmentDataImport> dataValid = new List<AnnualInstallmentDataImport>();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var dataItem in data)
                        {
                            ...
                            {
                                UnitPrice unitPrice = _context.UnitPricies.Where(m => m.Name == dataItem.UnitPriceName && m.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                                if (unitPrice == null)
                                {
                        ...
                        _context.AnnualInstallments.AddRange(dataValid);
        ...
        public static List<AnnualInstallmentDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<AnnualInstallmentDataImport> res = new List<AnnualInstallmentDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    AnnualInstallmentDataImport inputDetail = new AnnualInstallmentDataImport();
        ...
        //public async Task<IActionResult> ImportExcel([FromBody] List<AnnualInstallmentData> input)
        //{
        //    string accessToken = Request.Headers[HeaderNames.Authorization];
        //    Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
        //    if (token == null)
        ...
        //                    _context.AnnualInstallments.Add(item);
        //                    await _context.SaveChangesAsync();

        //                    LogActionModel logActionModel = new LogActionModel("Thêm mới lãi suất trả góp: " + item.Id, "AnnualInstallment", item.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        //                    LogAction logAction = _mapper.Map<LogAction>(logActionModel);
        //                    _context.Add(logAction);
        //                    await _context.SaveChangesAsync();
