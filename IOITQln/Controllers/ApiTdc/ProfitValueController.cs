...
namespace IOITQln.Controllers.ApiTdc
{
    ...
    [ApiController]
    public class ProfitValueController : ControllerBase
    {
        ...
        public ProfitValueController(ApiDbContext context, IMapper mapper, IHostingEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ProfitValue> data = _context.ProfitValues.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<ProfitValueData> res = _mapper.Map<List<ProfitValueData>>(data.ToList());
                        foreach (ProfitValueData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ProfitValue data = await _context.ProfitValues.FindAsync(id);
                ...
                def.meta = new Meta(200, ApiConstants.MessageResource.ACCTION_SUCCESS);
                ProfitValueData res = _mapper.Map<ProfitValueData>(data);
        ...
        // POST: api/ProfitValue
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ProfitValue input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ProfitValue)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ProfitValues.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số lãi phạt thuê: " + input.Id, "ProfitValue", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/ProfitValue/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ProfitValue input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ProfitValue data = await _context.ProfitValues.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa hệ số lãi phạt thuê: " + input.Id, "ProfitValue", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thành phần giá bán cấu thành: " + data.Id, "ProfitValue", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ProfitValueExists(int id)
        {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<ProfitValue> profitValues = _context.ProfitValues.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<ProfitValueData> mapper_dataTdc = _mapper.Map<List<ProfitValueData>>(profitValues);
            ...
            string template = @"templates/ProfitValue.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
            string templatePath = Path.Combine(webRootPath, template);

            MemoryStream ms = WriteDataToExcel(templatePath, 0, profitValues, mapper_dataTdc);
            byte[] byteArrayContent = ms.ToArray();
            return new FileContentResult(byteArrayContent, "application/octet-stream");
        }

        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<ProfitValue> data, List<ProfitValueData> datatdc)
        {
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.TdcProfitValue;

                List<ProfitValueDataImport> data = new List<ProfitValueDataImport>();
                ...
                List<ProfitValueDataImport> dataValid = new List<ProfitValueDataImport>();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.ProfitValues.AddRange(dataValid);
        ...
        public static List<ProfitValueDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<ProfitValueDataImport> res = new List<ProfitValueDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    ProfitValueDataImport inputDetail = new ProfitValueDataImport();
        ...
        //public async Task<IActionResult> ImportExcel([FromBody] List<ProfitValueData> input)
        //{
        //    string accessToken = Request.Headers[HeaderNames.Authorization];
        //    Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
        //    if (token == null)
        ...
        //                    _context.ProfitValues.Add(item);
        //                    await _context.SaveChangesAsync();

        //                    LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số lãi phạt thuê: " + item.Id, "ProfitValue", item.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        //                    LogAction logAction = _mapper.Map<LogAction>(logActionModel);
        //                    _context.Add(logAction);
        //                    await _context.SaveChangesAsync();
