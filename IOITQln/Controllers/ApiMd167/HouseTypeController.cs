...
namespace IOITQln.Controllers.ApiInv
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("Md167HouseType", "Md167HouseType");
        private static string functionCode = "HOUSE_TYPE";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public HouseTypeController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167HouseType> data = _context.Md167HouseTypes.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167HouseType data = await _context.Md167HouseTypes.FindAsync(id);
        ...
        // POST: api/Md167HouseType
        [HttpPost]
        public async Task<IActionResult> Post(Md167HouseType input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167HouseType)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167HouseTypes.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();
                        input.Code = CodeIndentity.CodeInd("LN", input.Id, 3);
                        _context.Update(input);
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm loại nhà " + input.Name, "HouseType", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167HouseTypeExists(input.Id))
                        {
        ...
        // PUT: api/Area/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167HouseType input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167HouseType data = await _context.Md167HouseTypes.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa loại nhà " + input.Name, "Md167HouseType", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167HouseTypeExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //Md167House
                Md167House house = _context.Md167Houses.Where(b => b.HouseTypeId == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (house != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa loại nhà " + data.Name, "Md167HouseType", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167HouseTypeExists(int id)
        {
            return _context.Md167HouseTypes.Count(e => e.Id == id) > 0;
        }
    }
}
