...
namespace IOITQln.Controllers.ApiInv
{
    ...
    [ApiController]
    public class TranferUnitController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("Auctioneer", "Auctioneer");
        private static string functionCode = "TRANFER_UNIT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public TranferUnitController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167TranferUnit> data = _context.Md167TranferUnits.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167TranferUnit data = await _context.Md167TranferUnits.FindAsync(id);
        ...
        // POST: api/Md167TranferUnit
        [HttpPost]
        public async Task<IActionResult> Post(Md167TranferUnit input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167TranferUnit)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                var codeExist = _context.Md167TranferUnits.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167TranferUnits.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới vùng khu " + input.Name, "Area", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167TranferUnitExists(input.Id))
                        {
        ...
        // PUT: api/Area/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167TranferUnit input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167TranferUnit data = await _context.Md167TranferUnits.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                var codeExist = _context.Md167TranferUnits.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != input.Id).FirstOrDefault();
                if (codeExist != null)
                {
                    def.meta = new Meta(211, "Mã đã tồn tại!");
                    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        await _context.SaveChangesAsync();
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa đơn vị chuyển giao " + input.Name, "Md167TranferUnit", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167TranferUnitExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //BlockDetail
                Md167House itemExist = _context.Md167Houses.Where(b => b.Md167TransferUnitId == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (itemExist != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (Md167House). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa đơn vị chuyển giao " + data.Name, "Md167TranferUnit", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167TranferUnitExists(int id)
        {
            return _context.Md167TranferUnits.Count(e => e.Id == id) > 0;
        }
    }
}
