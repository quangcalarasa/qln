...
namespace IOITQln.Controllers.ApiMd167
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("Md167DELEGATE", "Md167DELEGATE");
        private static string functionCode = "DELEGATE";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public DelegateController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167Delegate> data = _context.Md167Delegates.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167Delegate data = await _context.Md167Delegates.FindAsync(id);
        ...
        // POST: api/Md167Delegate
        [HttpPost]
        public async Task<IActionResult> Post(Md167Delegate input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167Delegate)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //var codeExist = _context.Md167Delegates.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167Delegates.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();
                        input.Code = CodeIndentity.CodeInd("TCCN",input.Id,5);
                        _context.Update(input);
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới đại diện cá nhân/ công ty " + input.Code, "Md167Delegate", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167DelegateExists(input.Id))
                        {
        ...
        // PUT: api/Delegate/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167Delegate input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167Delegate data = await _context.Md167Delegates.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thuế suất đất " + input.Code, "Md167Delegate", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167DelegateExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //Md167Contract
                Md167Contract itemExist = _context.Md167Contracts.Where(b => b.DelegateId == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (itemExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thuế suất đất " + data.Code, "Md167Delegate", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167DelegateExists(int id)
        {
            return _context.Md167Delegates.Count(e => e.Id == id) > 0;
        }
    }
}
