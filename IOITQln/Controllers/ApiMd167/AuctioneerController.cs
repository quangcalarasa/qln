...
namespace IOITQln.Controllers.ApiInv
{
    ...
    public class AuctioneerController : ControllerBase
    {
        ...
        private static string functionCode = "AUCTIONEER";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public AuctioneerController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167Auctioneer> data = _context.Md167Auctioneers.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167Auctioneer data = await _context.Md167Auctioneers.FindAsync(id);
        ...
        // POST: api/Md167Auctioneer
        [HttpPost]
        public async Task<IActionResult> Post(Md167Auctioneer input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167Auctioneer)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                if (input.UnitName == null || input.UnitName == "")
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167Auctioneers.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();
                        input.Code = CodeIndentity.CodeInd("DVDG", input.Id, 5);
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới vùng khu " + input.UnitName, "Area", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167AuctioneerExists(input.Id))
                        {
        ...
        // PUT: api/Area/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167Auctioneer input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167Auctioneer data = await _context.Md167Auctioneers.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                //var codeExist = _context.Md167Auctioneers.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        await _context.SaveChangesAsync();
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa đơn vị đấu giá " + input.UnitName, "Md167Auctioneer", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167AuctioneerExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa đơn vị đấu giá " + data.UnitName, "Md167Auctioneer", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167AuctioneerExists(int id)
        {
            return _context.Md167Auctioneers.Count(e => e.Id == id) > 0;
        }
    }
}
