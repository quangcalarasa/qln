...
namespace IOITQln.Controllers.ApiInv
{
    ...
    [ApiController]
    public class ManagePurposeController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ManagePurpose", "ManagePurpose");
        private static string functionCode = "VAT_VALUE";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ManagePurposeController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167ManagePurpose> data = _context.Md167ManagePurposes.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167ManagePurpose data = await _context.Md167ManagePurposes.FindAsync(id);
        ...
        // POST: api/Md167ManagePurpose
        [HttpPost]
        public async Task<IActionResult> Post(Md167ManagePurpose input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167ManagePurpose)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167ManagePurposes.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới giá trị VAT ", "Md167ManagePurpose", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167ManagePurposeExists(input.Id))
                        {
        ...
        // PUT: api/Area/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167ManagePurpose input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167ManagePurpose data = await _context.Md167ManagePurposes.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa giá trị VAT", "Md167ManagePurpose", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167ManagePurposeExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //Kiểm tra dữ liệu liên quan có tồn tại thì không thể xóa
                //Md167House
                Md167House itemExist = _context.Md167Houses.Where(b => b.PurposeUsing == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (itemExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa giá trị VAT", "Md167ManagePurpose", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167ManagePurposeExists(int id)
        {
            return _context.Md167ManagePurposes.Count(e => e.Id == id) > 0;
        }
    }
}
