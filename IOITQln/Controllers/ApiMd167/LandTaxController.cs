...
namespace IOITQln.Controllers.ApiMd167
{
    ...
    [ApiController]
    public class LandTaxController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("Md167LandTax", "Md167LandTax");
        private static string functionCode = "DM_TAX_LAND";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public LandTaxController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167LandTax> data = _context.Md167LandTaxs.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    else
                    {
                        ...
                        //}
                        //def.data = res;
                        Md167LandTaxRes res = new Md167LandTaxRes
                        {
                            StateDefault = _context.Md167LandTaxs.Any(x => x.IsDefault == true&&x.Status!=AppEnums.EntityStatus.DELETED),
                            md167LandTaxs = data.ToList(),
                        };
                        def.data = res;
                    }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Md167LandTax data = await _context.Md167LandTaxs.FindAsync(id);
        ...
        // POST: api/Md167LandTax
        [HttpPost]
        public async Task<IActionResult> Post(Md167LandTax input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167LandTax)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //var codeExist = _context.Md167LandTaxs.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167LandTaxs.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();
                        input.Code = CodeIndentity.CodeInd("TS", input.Id, 4);
                        _context.Update(input);

                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới thuế suất đất " + input.Code, "Md167LandTax", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167LandTaxExists(input.Id))
                        {
        ...
        // PUT: api/Area/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167LandTax input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167LandTax data = await _context.Md167LandTaxs.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thuế suất đất " + input.Code, "Md167LandTax", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //house
                Md167House house = _context.Md167Houses.Where(b => b.LandTaxRate == id && b.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (house != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (house). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thuế suất đất " + data.Code, "Md167LandTax", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool Md167LandTaxExists(int id)
        {
            return _context.Md167LandTaxs.Count(e => e.Id == id) > 0;
        }
    }
}
