...
namespace IOITQln.Controllers.ApiMd167
{
    ...
    [ApiController]
    public class Md167HouseFileController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("Md167House", "Md167House");
        private readonly ApiDbContext _context;
        string functionCode = "HOUSE";
        private IMapper _mapper;

        public Md167HouseFileController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Md167HouseFile> data = _context.Md167HouseFiles.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Md167HouseFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Md167HouseFile)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                List<Md167HouseFile> item = _context.Md167HouseFiles.Where(l => l.Status != AppEnums.EntityStatus.DELETED).ToList();

                Md167House md167House = _context.Md167Houses.Where(x => x.Id == input.Md167HouseId).FirstOrDefault();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Md167HouseFiles.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới tệp đính kèm: " + input.Id, "Md167HouseFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (Md167HouseFileExists(input.Id))
                        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Md167HouseFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Md167HouseFile data = await _context.Md167HouseFiles.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa : " + input.Id, "Md167HouseFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!Md167HouseFileExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                Md167HouseFile data = await _context.Md167HouseFiles.FindAsync(id);
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa " + data.Id, "Md167HouseFile", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        [HttpGet("getFile/{md167HouseId}")]
        public async Task<IActionResult> GetPayBymd167HouseId(int md167HouseId)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                var data = await _context.Md167HouseFiles.Where(f => f.Md167HouseId == md167HouseId && f.Status != AppEnums.EntityStatus.DELETED).ToListAsync();
                if (data == null)
                {
        ...
        private bool Md167HouseFileExists(int id)
        {
            return _context.Md167HouseFiles.Count(e => e.Id == id) > 0;
        }
    }
}
