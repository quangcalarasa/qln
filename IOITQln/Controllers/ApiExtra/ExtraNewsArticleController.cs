...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraNewsArticleController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("NewsArticle", "NewsArticle");
        private static string functionCode = "NewsArticle";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraNewsArticleController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraNewsArticle> data = _context.ExtraNewsArticles.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<ExtraNewsArticleData> res = _mapper.Map<List<ExtraNewsArticleData>>(data.ToList());
                        foreach (ExtraNewsArticleData item in res)
                        {
                            item.TypeNewsName = _context.ExtraNewsArticleLists.Where(b => b.Id == item.ExtraNewsArticleListId).Select(p => p.TypeNews).FirstOrDefault();
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraNewsArticle data = await _context.ExtraNewsArticles.FindAsync(id);
        ...
        // POST: api/ExtraNewsArticle
        [HttpPost]
        public async Task<IActionResult> Post(ExtraNewsArticle input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraNewsArticle)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraNewsArticles.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới" + input.ArticleTitle, "ExtraNewsArticle", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraNewsArticleExists(input.Id))
                        {
        ...
        // PUT: api/ExtraNewsArticle/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraNewsArticle input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraNewsArticle data = await _context.ExtraNewsArticles.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa  " + input.ArticleTitle, "ExtraNewsArticle", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraNewsArticleExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa  " + data.ArticleTitle, "ExtraNewsArticle", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraNewsArticleExists(int id)
        {
            return _context.ExtraNewsArticles.Count(e => e.Id == id) > 0;
        }
    }
}
