...
namespace IOITQln.Controllers.ApiExtra
{
    ...
    [ApiController]
    public class ExtraSupportRequestController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("support-requests", "support-requests");
        private static string functionCode = "SUPPORT_REQUESTS";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraSupportRequestController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraSupportRequest> data = _context.ExtraSupportRequests.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraSupportRequest data = await _context.ExtraSupportRequests.FindAsync(id);
        ...
        [HttpPost]
        public async Task<IActionResult> Post(ExtraSupportRequest input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraSupportRequest)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                if (input.TypeSupport == null)
                {
                    def.meta = new Meta(400, "Loại yêu cầu hỗ trợ không được để trống!");
                    return Ok(def);
                }

                if (input.Content == null)
                {
                    def.meta = new Meta(400, "Nội dung không được để trống!");
                    return Ok(def);
                }

                if (input.RequirePerson == null)
                {
                    def.meta = new Meta(400, "Người yêu cầu hỗ trợ không được để trống!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraSupportRequests.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới yêu cầu hỗ trợ: " + input.Code, "ExtraSupportRequest", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraSupportRequestExists(input.Id))
                        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraSupportRequest input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraSupportRequest data = await _context.ExtraSupportRequests.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa yêu cầu hỗ trợ: " + input.Code, "ExtraSupportRequest", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraSupportRequestExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa Yêu cầu hỗ trợ: " + data.Code, "ExtraSupportRequest", data.Id, HttpContext.Connection.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraSupportRequestExists(int id)
        {
            return _context.ExtraSupportRequests.Count(e => e.Id == id) > 0;
        }
    }
}
