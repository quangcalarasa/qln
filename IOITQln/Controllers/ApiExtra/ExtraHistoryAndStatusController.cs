...
namespace IOITQln.Controllers.ApiExtra
{
    ...
    [ApiController]
    public class ExtraHistoryAndStatusController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("track-house", "track-house");
        private static string functionCode = "TTLSSDCN";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private readonly IConfiguration _configuration;

        public ExtraHistoryAndStatusController(ApiDbContext context, IMapper mapper, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraHistoryAndStatus> data = _context.ExtraHistoryAndStatuses.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraHistoryAndStatus data = await _context.ExtraHistoryAndStatuses.FindAsync(id);
        ...
        [HttpPost]
        public async Task<IActionResult> Post(ExtraHistoryAndStatus input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraHistoryAndStatus)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraHistoryAndStatuses.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới yêu cầu hỗ trợ: " + input.Code, "ExtraHistoryAndStatus", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraHistoryAndStatusExists(input.Id))
                        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraHistoryAndStatus input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraHistoryAndStatus data = await _context.ExtraHistoryAndStatuses.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa yêu cầu hỗ trợ: " + input.Code, "ExtraHistoryAndStatus", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraHistoryAndStatusExists(data.Id))
                        {
        ...
        public async Task<ActionResult> ChangeStatusUser(int id, byte stt)
        {
            ...
            try
            {
                ...
                if (data == null)
                {
                    def.meta = new Meta(404, "Không tìm thấy lịch sử, tình trạng sử dụng nhà!");
                    return Ok(def);
                }
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    {
                        //thêm LogAction
                        string actionName = (AppEnums.EntityStatus)stt == AppEnums.EntityStatus.NA ? "Mở khóa " : ((AppEnums.EntityStatus)stt == AppEnums.EntityStatus.DELETED ? "Khóa " : "Đổi trạng thái");
                        LogActionModel logActionModel = new LogActionModel(actionName + "lịch sử, tình trạng sử dụng nhà " + data.Code, "User", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        ...
                        if ((AppEnums.EntityStatus)stt == AppEnums.EntityStatus.NA || (AppEnums.EntityStatus)stt == AppEnums.EntityStatus.DELETED)
                        {
                            success = true;
                        }
                        else
                        ...
                        {
                            transaction.Commit();
                            def.meta = new Meta(200, actionName + " cho lịch sử, tình trạng sử dụng nhà thành công!");
                            def.data = data;
                            return Ok(def);
                        }
                        else
                        {
                            transaction.Rollback();
                            def.meta = new Meta(500, actionName + " cho lịch sử, tình trạng sử dụng nhà không thành công!");
                            def.data = data;
                            return Ok(def);
                        }
                    }
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa Yêu cầu hỗ trợ: " + data.Code, "ExtraHistoryAndStatus", data.Id, HttpContext.Connection.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraHistoryAndStatusExists(int id)
        {
            return _context.ExtraHistoryAndStatuses.Count(e => e.Id == id) > 0;
        }
