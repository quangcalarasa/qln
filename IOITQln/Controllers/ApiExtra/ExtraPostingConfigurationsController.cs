...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraPostingConfigurationsController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("PostingConfiguration", "PostingConfiguration");
        private static string functionCode = "PostingConfiguration";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraPostingConfigurationsController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraPostingConfiguration> data = _context.ExtraPostingConfigurations.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        // GET: api/ExtraPostingConfiguration/1
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraPostingConfiguration data = await _context.ExtraPostingConfigurations.FindAsync(id);
        ...
        // POST: api/ExtraPostingConfiguration
        [HttpPost]
        public async Task<IActionResult> Post(ExtraPostingConfiguration input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraPostingConfiguration)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraPostingConfigurations.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới" + input.CapacityFile, "ExtraPostingConfiguration", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraPostingConfigurationExists(input.Id))
                        {
        ...
        // PUT: api/ExtraPostingConfiguration/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraPostingConfiguration input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraPostingConfiguration data = await _context.ExtraPostingConfigurations.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa  " + input.CapacityFile, "ExtraPostingConfiguration", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraPostingConfigurationExists(data.Id))
                        {
        ...
        // DELETE: api/ExtraPostingConfiguration/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa  " + data.CapacityFile, "ExtraPostingConfiguration", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraPostingConfigurationExists(int id)
        {
            return _context.ExtraPostingConfigurations.Count(e => e.Id == id) > 0;
        }
    }
}
