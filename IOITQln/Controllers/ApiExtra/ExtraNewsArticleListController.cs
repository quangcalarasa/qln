...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraNewsArticleListController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("NewsArticleList", "NewsArticleList");
        private static string functionCode = "NewsArticleList";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraNewsArticleListController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraNewsArticleList> data = _context.ExtraNewsArticleLists.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        // GET: api/ExtraNewsArticleList/1
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraNewsArticleList data = await _context.ExtraNewsArticleLists.FindAsync(id);
        ...
        // POST: api/ExtraNewsArticleList
        [HttpPost]
        public async Task<IActionResult> Post(ExtraNewsArticleList input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraNewsArticleList)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                if (input.TypeNews == null || input.TypeNews == "")
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraNewsArticleLists.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới" + input.TypeNews, "ExtraNewsArticleList", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraNewsArticleListExists(input.Id))
                        {
        ...
        // PUT: api/ExtraNewsArticleList/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraNewsArticleList input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraNewsArticleList data = await _context.ExtraNewsArticleLists.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa  " + input.TypeNews, "ExtraNewsArticleList", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraNewsArticleListExists(data.Id))
                        {
        ...
        // DELETE: api/ExtraNewsArticleList/1
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                var typeNews = _context.ExtraNewsArticles.Where(p => p.ExtraNewsArticleListId == data.Id && p.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if(typeNews != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (ExtraNewsArticles). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa  " + data.TypeNews, "ExtraNewsArticleList", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraNewsArticleListExists(int id)
        {
            return _context.ExtraNewsArticleLists.Count(e => e.Id == id) > 0;
        }
    }
}
