...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraTemplateController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ExtraTemplate", "ExtraTemplate");
        private static string functionCode = "AREA_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraTemplateController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraTemplate> data = _context.ExtraTemplates.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraTemplate data = await _context.ExtraTemplates.FindAsync(id);
        ...
        // POST: api/ExtraTemplate
        [HttpPost]
        public async Task<IActionResult> Post(ExtraTemplate input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraTemplate)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //ExtraTemplate codeExist = _context.ExtraTemplates.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraTemplates.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới template" + input.Name, "ExtraTemplate", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraTemplateExists(input.Id))
                        {
        ...
        // PUT: api/ExtraTemplate/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraTemplate input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraTemplate data = await _context.ExtraTemplates.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                //ExtraTemplate codeExist = _context.ExtraTemplates.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                //if (codeExist != null)
                //{
                //    def.meta = new Meta(211, "Mã đã tồn tại!");
                //    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa template" + input.Name, "ExtraTemplate", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraTemplateExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                //ExtraEmailDebt
                ExtraEmailDebt item = _context.ExtraEmailDebts.Where(a => a.TemplateId == id && a.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (item != null)
                {
                    def.meta = new Meta(222, "Dữ liệu liên quan còn tồn tại (ExtraEmailDebt). Không thể xóa bản ghi này!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa template" + data.Name, "ExtraTemplate", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraTemplateExists(int id)
        {
            return _context.ExtraTemplates.Count(e => e.Id == id) > 0;
        }
    }
}
