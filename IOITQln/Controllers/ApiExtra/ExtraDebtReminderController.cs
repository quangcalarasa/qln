...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraDebtReminderController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ExtraDebtReminder", "ExtraDebtReminder");
        private static string functionCode = "AREA_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraDebtReminderController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraDebtReminder> data = _context.ExtraDebtReminders.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraDebtReminder data = await _context.ExtraDebtReminders.FindAsync(id);
        ...
        // POST: api/ExtraDebtReminder
        [HttpPost]
        public async Task<IActionResult> Post(ExtraDebtReminder input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraDebtReminder)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //ExtraDebtReminder codeExist = _context.ExtraDebtReminders.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraDebtReminders.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới nhắc nợ" + input.Code, "ExtraDebtReminder", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraDebtReminderExists(input.Id))
                        {
        ...
        // PUT: api/ExtraDebtReminder/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraDebtReminder input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraDebtReminder data = await _context.ExtraDebtReminders.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                //ExtraDebtReminder codeExist = _context.ExtraDebtReminders.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                //if (codeExist != null)
                //{
                //    def.meta = new Meta(211, "Mã đã tồn tại!");
                //    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa nhắc nợ" + input.Code, "ExtraDebtReminder", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraDebtReminderExists(data.Id))
                        {
        ...
        // DELETE: api/ExtraDebtReminder/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa nhắc nợ" + data.Code, "ExtraDebtReminder", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraDebtReminderExists(int id)
        {
            return _context.ExtraDebtReminders.Count(e => e.Id == id) > 0;
        }
    }
}
