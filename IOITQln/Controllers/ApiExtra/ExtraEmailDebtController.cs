...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraEmailDebtController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ExtraEmailDebt", "ExtraEmailDebt");
        private static string functionCode = "AREA_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public ExtraEmailDebtController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ExtraEmailDebt> data = _context.ExtraEmailDebts.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<ExtraEmailDebtData> res = _mapper.Map<List<ExtraEmailDebtData>>(data.ToList());
                        foreach (ExtraEmailDebtData item in res)
                        {
                            ExtraTemplate extraTemplate = _context.ExtraTemplates.Where(b => b.Id == item.TemplateId).FirstOrDefault();
                            if (extraTemplate != null)
                            {
                                item.TemplateName = extraTemplate.Name;
                            }
                        }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ExtraEmailDebt data = await _context.ExtraEmailDebts.FindAsync(id);
        ...
        // POST: api/ExtraEmailDebt
        [HttpPost]
        public async Task<IActionResult> Post(ExtraEmailDebt input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraEmailDebt)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //ExtraEmailDebt codeExist = _context.ExtraEmailDebts.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraEmailDebts.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới email" + input.Header, "ExtraEmailDebt", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ExtraEmailDebtExists(input.Id))
                        {
        ...
        // PUT: api/ExtraEmailDebt/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ExtraEmailDebt input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ExtraEmailDebt data = await _context.ExtraEmailDebts.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                //ExtraEmailDebt codeExist = _context.ExtraEmailDebts.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                //if (codeExist != null)
                //{
                //    def.meta = new Meta(211, "Mã đã tồn tại!");
                //    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa email" + input.Header, "ExtraEmailDebt", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ExtraEmailDebtExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa email" + data.Header, "ExtraEmailDebt", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ExtraEmailDebtExists(int id)
        {
            return _context.ExtraEmailDebts.Count(e => e.Id == id) > 0;
        }
    }
}
