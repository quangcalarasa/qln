...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ExtraEmailNotiController : ControllerBase
    {
        ...
        public ExtraEmailNotiController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<IActionResult> Get()
        {
            ...
            try
            {
                ExtraEmailNoti data = _context.ExtraEmailNotis.Where(r => r.Status != EntityStatus.DELETED).OrderByDescending(x => x.CreatedAt).FirstOrDefault();

                if (data == null)
                {
                    data = new ExtraEmailNoti();
                }
        ...
        // POST: api/Area1
        [HttpPost]
        public async Task<IActionResult> Post(ExtraEmailNoti input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ExtraEmailNoti)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ExtraEmailNotis.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Update config" + input.PortSentEmail, "ExtraEmailNoti", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
