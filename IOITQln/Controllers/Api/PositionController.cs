...
namespace IOITQln.Controllers.Api
{
    ...
    public class PositionController : ControllerBase
    {
        ...
        private static string functionCode = "POSITION_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public PositionController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Position> data = _context.Positions.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        // POST: api/Position
        [HttpPost]
        public async Task<IActionResult> Post(Position input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Position)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                Position codeExist = _context.Positions.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới chức vụ " + input.Name, "Position", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/Position/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Position input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Position data = await _context.Positions.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                Position codeExist = _context.Positions.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                if (codeExist != null)
                {
                    def.meta = new Meta(211, "Mã đã tồn tại!");
                    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa chức vụ " + input.Name, "Position", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa chức vụ " + data.Name, "Position", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        {
            return _context.Positions.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Common_Position;

                List<PositionDataImport> data = new List<PositionDataImport>();

                List<SimpleData> positionData = _context.Positions.Where(e => e.Status != EntityStatus.DELETED).Select(e => new SimpleData { Id = e.Id, Name = e.Name }).ToList();
                List<SimpleData> departmentData = _context.Departments.Where(e => e.Status != EntityStatus.DELETED).Select(e => new SimpleData { Id = e.Id, Name = e.Name }).ToList();
                    ...
                    if (postedFile != null && postedFile.Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            using (var binaryReader = new BinaryReader(file.OpenReadStream()))
                            {
                                ...
                                {
                                    data = importData(ms, 0, 1, positionData, departmentData);
                                }
                            }
                        }
                    }
                ...
                List<PositionDataImport> dataValid = data.Where(e => e.Valid).ToList();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.Positions.AddRange(dataValid);
        ...
        public static List<PositionDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart, List<SimpleData> positionData, List<SimpleData> departmentData)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<PositionDataImport> res = new List<PositionDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    PositionDataImport input1Detai = new PositionDataImport();
                            ...
                            else if (i == 1)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        ...
                                        SimpleData existCode = positionData.Where(e => e.Code == input1Detai.Code).FirstOrDefault();
                                        if(existCode != null)
                                        {
                                            input1Detai.Valid = false;
                                            input1Detai.ErrMsg += "Cột Mã chức danh đã tồn tại\n";
                                        }
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Mã chức danh chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Mã chức danh\n";
                                }
                            }
                            else if (i == 2)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Tên chức danh chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Tên chức danh\n";
                                }
                            }
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.DepartmentName = str;

                                        //Kiểm tra phòng ban có hợp lệ hay không
                                        string strNoneUnicode = UtilsService.NonUnicode(str);
                                        SimpleData validDepartment = departmentData.AsEnumerable().Where(e => UtilsService.NonUnicode(e.Name) == strNoneUnicode).FirstOrDefault();
                                        if(validDepartment == null)
                                        {
                                            input1Detai.Valid = false;
                                            input1Detai.ErrMsg += "Không tìm thấy phòng ban\n";
                                        }
                                        else
                                        {
                                            input1Detai.DepartmentId = (int)validDepartment.Id;
                                        }
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Phòng ban\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AuthorDocsCode = str;
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Mã văn bản ủy quyền\n";
                                }
                            }
                            else if (i == 5)
                            {
