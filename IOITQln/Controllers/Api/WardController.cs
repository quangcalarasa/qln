...
namespace IOITQln.Controllers.Api
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("ward", "ward");
        private static string functionCode = "WARD_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public WardController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Ward> data;

                    if(moduleSystem != (int)ModuleSystem.NOC)
                    {
                        data = _context.Wards.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    }
                    else
                    {
                        data = _context.Wards.Where(c => c.ProvinceId == 2 && c.Status != AppEnums.EntityStatus.DELETED);
                    }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Ward data = await _context.Wards.FindAsync(id);
        ...
        // POST: api/Ward
        [HttpPost]
        public async Task<IActionResult> Post(Ward input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Ward)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                //Ward codeExist = _context.Wards.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                //if (codeExist != null)
                //{
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới phường/xã " + input.Name, "Ward", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/Ward/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Ward input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Ward data = await _context.Wards.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                //Ward codeExist = _context.Wards.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                //if (codeExist != null)
                //{
                //    def.meta = new Meta(211, "Mã đã tồn tại!");
                //    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa phường/xã " + input.Name, "Ward", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> UpdateName(int id, [FromBody] Ward input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    string district = _context.Districts.FirstOrDefault(x => x.Id == data.DistrictId)?.Name ?? "";
                    input.InfoValue.Add(new Lane.OldName { Name = data.Name + ", " + district + ", " + provine });
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thông tin Phường/xã " + input.Name, "Ward", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa phường/xã " + data.Name, "Ward", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        {
            return _context.Wards.Count(e => e.Id == id) > 0;
        }
    }
}
