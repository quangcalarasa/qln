...
namespace IOITQln.Controllers.Api
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("Holiday", "Holiday");
        private static string functionCode = "HOLIDAY_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public HolidayController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Holiday> data = _context.Holidays.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Holiday data = await _context.Holidays.FindAsync(id);
        ...
        // POST: api/Holiday
        [HttpPost]
        public async Task<IActionResult> Post(Holiday input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Holiday)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                {
                    def.meta = new Meta(400, "Ngày bắt đầu và ngày kết thúc không được để trống!");
                    return Ok(def);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới kỳ nghỉ lễ " + input.Name, "Holiday", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/Holiday/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Holiday input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Holiday data = await _context.Holidays.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa kỳ nghỉ lễ " + input.Name, "Holiday", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa kỳ nghỉ lễ " + data.Name, "Holiday", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        //Get Holiday by Month and Year
        [HttpGet("getByMonthAndYear/{month}/{year}")]
        public async Task<IActionResult> GetById(int month, int year)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                TimeSpan firstDay = new TimeSpan(0, 0, 0);
                TimeSpan endDay = new TimeSpan(23, 59, 59);

                var startDate = new DateTime(year, month, 1) + firstDay;
                var endDate = startDate.AddMonths(1).AddDays(-1) + endDay;

                List<Holiday> data = await _context.Holidays.Where(h => !(h.StartDate < startDate && h.EndDate < startDate) && !(h.StartDate > endDate && h.EndDate > endDate)).ToListAsync();
        ...
        {
            return _context.Holidays.Count(e => e.Id == id) > 0;
        }
    }
}
