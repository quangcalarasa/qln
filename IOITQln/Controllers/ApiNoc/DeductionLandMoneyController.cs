...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class DeductionLandMoneyController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("DeductionLandMoney", "DeductionLandMoney");
        private static string functionCode = "DEDUCTION_LAND_MONEY_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public DeductionLandMoneyController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<DeductionLandMoney> data = _context.DeductionLandMonies.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<DeductionLandMoneyData> res = _mapper.Map<List<DeductionLandMoneyData>>(data.ToList());
                        foreach (DeductionLandMoneyData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                DeductionLandMoney data = await _context.DeductionLandMonies.FindAsync(id);
        ...
        // POST: api/DeductionLandMoney
        [HttpPost]
        public async Task<IActionResult> Post(DeductionLandMoney input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (DeductionLandMoney)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.DeductionLandMonies.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới thông tin tiền đất miễn giảm", "DeductionLandMoney", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (DeductionLandMoneyExists(input.Id))
                        {
        ...
        // PUT: api/DeductionLandMoney/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] DeductionLandMoney input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                DeductionLandMoney data = await _context.DeductionLandMonies.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thông tin tiền đất miễn giảm", "DeductionLandMoney", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!DeductionLandMoneyExists(data.Id))
                        {
        ...
        // DELETE: api/DeductionLandMoney/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thông tin tiền đất miễn giảm ", "DeductionLandMoney", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool DeductionLandMoneyExists(int id)
        {
            return _context.DeductionLandMonies.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Deduction_Land_Money;

                List<DeductionLandMoneyDataImport> data = new List<DeductionLandMoneyDataImport>();
                ...
                List<DeductionLandMoneyDataImport> dataValid = new List<DeductionLandMoneyDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.DeductionLandMonies.AddRange(dataValid);
        ...
        public static List<DeductionLandMoneyDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<DeductionLandMoneyDataImport> res = new List<DeductionLandMoneyDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    DeductionLandMoneyDataImport input1Detai = new DeductionLandMoneyDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Condition = str;
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Điều kiện miễn giảm chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Điều kiện miễn giảm\n";
                                }
                            }
                            else if (i == 4)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<DeductionLandMoney> data = _context.DeductionLandMonies.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/tien_dat_mien_giam.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<DeductionLandMoney> data, List<Decree> decrees)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.Condition);
                                }
                                else if (i == 4)
                                {
