...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class LandSpecialCoefficientController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("LandSpecialCoefficient", "LandSpecialCoefficient");
        private static string functionCode = "LAND_SPECIAL_COEFFICIENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public LandSpecialCoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<LandSpecialCoefficient> data = _context.LandSpecialCoefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<LandSpecialCoefficientData> res = _mapper.Map<List<LandSpecialCoefficientData>>(data.ToList());
                        foreach (LandSpecialCoefficientData item in res)
                        {
        ...
        // GET: api/LandSpecialCoefficient/1
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                LandSpecialCoefficient data = await _context.LandSpecialCoefficients.FindAsync(id);
        ...
        // POST: api/LandSpecialCoefficient
        [HttpPost]
        public async Task<IActionResult> Post(LandSpecialCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (LandSpecialCoefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.LandSpecialCoefficients.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số khu đất, thửa đất có hình dáng đặc biệt ", "LandSpecialCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (LandSpecialCoefficientExists(input.Id))
                        {
        ...
        // PUT: api/LandSpecialCoefficient/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] LandSpecialCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                LandSpecialCoefficient data = await _context.LandSpecialCoefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thông tin hệ số khu đất, thửa đất có hình dáng đặc biệt ", "LandSpecialCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!LandSpecialCoefficientExists(data.Id))
                        {
        ...
        // DELETE: api/LandSpecialCoefficient/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thông tin hệ số khu đất, thửa đất có hình dáng đặc biệt ", "LandSpecialCoefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool LandSpecialCoefficientExists(int id)
        {
            return _context.LandSpecialCoefficients.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Land_Special_Coefficient;

                List<LandSpecialCoefficientDataImport> data = new List<LandSpecialCoefficientDataImport>();
                ...
                List<LandSpecialCoefficientDataImport> dataValid = new List<LandSpecialCoefficientDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.LandSpecialCoefficients.AddRange(dataValid);
        ...
        public static List<LandSpecialCoefficientDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<LandSpecialCoefficientDataImport> res = new List<LandSpecialCoefficientDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    LandSpecialCoefficientDataImport input1Detai = new LandSpecialCoefficientDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        ...
                                        if (input1Detai.DoApply.Value.Year < 1900)
                                        {
                                            input1Detai.Valid = false;
                            ...
                            else if (i == 4)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value1 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Có phần S không mặt tiền đường (hẻm) từ 15m2 trở lên chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Có phần S không mặt tiền đường (hẻm) từ 15m2 trở lên\n";
                                }
                            }
                            else if (i == 5)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value2 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột 5R< D <=8R chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột 5R< D <=8R\n";
                                }
                            }
                            else if (i == 6)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value3 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột D >8R chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột D >8R\n";
                                }
                            }
                            else if (i == 7)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value4 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Đoạn đường nằm hai bên dạ cầu (song song cầu) chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Đoạn đường nằm hai bên dạ cầu (song song cầu)\n";
                                }
                            }
                            else if (i == 8)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value5 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Đoạn đường nằm hai bên cầu vượt (song song cầu) chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Đoạn đường nằm hai bên cầu vượt (song song cầu)\n";
                                }
                            }
                            else if (i == 9)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value6 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Nằm trong hành lang bảo vệ của đường điện cao thế chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Nằm trong hành lang bảo vệ của đường điện cao thế\n";
                                }
                            }
                            else if (i == 10)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value7 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Đường nhánh dẫn lên cầu vượt chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Đường nhánh dẫn lên cầu vượt\n";
                                }
                            }
                            else if (i == 11)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value8 = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Cách lề đường bằng một con kênh, rạch không được san lấp chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Cách lề đường bằng một con kênh, rạch không được san lấp\n";
                                }
                            }
                        }
                        catch (Exception ex)
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<LandSpecialCoefficient> data = _context.LandSpecialCoefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/he_so_khu_dat_dac_biet.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<LandSpecialCoefficient> data, List<Decree> decrees)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.DoApply != null ? item.DoApply.Value.ToString("dd/MM/yyyy") : "");
                                }
                                else if (i == 4)
                                {
                                    row.GetCell(i).SetCellValue(item.Value1);
                                }
                                else if (i == 5)
                                {
                                    row.GetCell(i).SetCellValue(item.Value2);
                                }
                                else if (i == 6)
                                {
                                    row.GetCell(i).SetCellValue(item.Value3);
                                }
                                else if (i == 7)
                                {
                                    row.GetCell(i).SetCellValue(item.Value4);
                                }
                                else if (i == 8)
                                {
                                    row.GetCell(i).SetCellValue(item.Value5);
                                }
                                else if (i == 9)
                                {
                                    row.GetCell(i).SetCellValue(item.Value6);
                                }
                                else if (i == 10)
                                {
                                    row.GetCell(i).SetCellValue(item.Value7);
                                }
                                else if (i == 11)
                                {
                                    row.GetCell(i).SetCellValue(item.Value8);
                                }
                            }
                            rowStart++;
                            k++;
