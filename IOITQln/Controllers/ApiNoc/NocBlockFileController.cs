...
namespace IOITQln.Controllers.ApiNoc
{
    ...
    [ApiController]
    public class NocBlockFileController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("block", "block");
        private readonly ApiDbContext _context;
        string functionCode = "BLOCK_MANAGEMENT";
        private IMapper _mapper;

        public NocBlockFileController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<NocBlockFile> data = _context.NocBlockFiles.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NocBlockFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (NocBlockFile)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                List<NocBlockFile> item = _context.NocBlockFiles.Where(l => l.Status != AppEnums.EntityStatus.DELETED).ToList();

                Block block = _context.Blocks.Where(x => x.Id == input.NocBlockId).FirstOrDefault();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.NocBlockFiles.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới tệp đính kèm: " + input.Id, "NocBlockFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (NocBlockFileExists(input.Id))
                        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NocBlockFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                NocBlockFile data = await _context.NocBlockFiles.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa : " + input.Id, "NocBlockFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                NocBlockFile data = await _context.NocBlockFiles.FindAsync(id);
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa " + data.Id, "NocBlockFile", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        [HttpGet("getFile/{nocBlockId}")]
        public async Task<IActionResult> GetPayBynocBlockId(int nocBlockId)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                var data = await _context.NocBlockFiles.Where(f => f.NocBlockId == nocBlockId && f.Status != AppEnums.EntityStatus.DELETED).ToListAsync();
                if (data == null)
                {
        ...
        private bool NocBlockFileExists(int id)
        {
