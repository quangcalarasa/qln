...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class ConstructionPriceController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("ConstructionPrice", "ConstructionPrice");
        private static string functionCode = "CONSTRUCTION_PRICE_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public ConstructionPriceController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<ConstructionPrice> data = _context.ConstructionPricies.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<ConstructionPriceData> res = _mapper.Map<List<ConstructionPriceData>>(data.ToList());
                        foreach (ConstructionPriceData item in res)
                        {
                            ...
                            //item.constructionPriceItems = _context.ConstructionPriceItems.Where(c => c.ConstructionPriceId == item.Id && c.Status != AppEnums.EntityStatus.DELETED).ToList();
                        }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                ConstructionPrice data = await _context.ConstructionPricies.FindAsync(id);
        ...
        // POST: api/ConstructionPrice
        [HttpPost]
        public async Task<IActionResult> Post(ConstructionPriceData input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (ConstructionPriceData)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.ConstructionPricies.Add(input);

                    try
                    {
                        await _context.SaveChangesAsync();

                        //ConstructionPriceItem
                        //if (input.constructionPriceItems != null)
                        //{
                        //    foreach (var constructionPriceItem in input.constructionPriceItems)
                        //    {
                        //        constructionPriceItem.ConstructionPriceId = input.Id;
                        //        constructionPriceItem.CreatedBy = fullName;
                        //        constructionPriceItem.CreatedById = userId;

                        //        _context.ConstructionPriceItems.Add(constructionPriceItem);
                        //    }
                        //}

                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới Chỉ số giá xây dựng công trình cho loại công trình " + input.NameOfConstruction, "ConstructionPrice", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (ConstructionPriceExists(input.Id))
                        {
        ...
        // PUT: api/ConstructionPrice/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ConstructionPriceData input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                ConstructionPrice data = await _context.ConstructionPricies.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //ConstructionPriceITem
                        //List<ConstructionPriceItem> constructionPriceItems = _context.ConstructionPriceItems.Where(l => l.ConstructionPriceId == input.Id && l.Status != AppEnums.EntityStatus.DELETED).AsNoTracking().ToList();
                        //if (input.constructionPriceItems != null)
                        //{
                        //    foreach (var constructionPriceItem in input.constructionPriceItems)
                        //    {
                        //        ConstructionPriceItem constructionPriceItemExist = constructionPriceItems.Where(l => l.Id == constructionPriceItem.Id).FirstOrDefault();
                        //        if (constructionPriceItemExist == null)
                        //        {
                        //            constructionPriceItem.ConstructionPriceId = input.Id;
                        //            constructionPriceItem.CreatedBy = fullName;
                        //            constructionPriceItem.CreatedById = userId;
                        ...
                        //        {
                        //            constructionPriceItem.ConstructionPriceId = input.Id;
                        //            constructionPriceItem.CreatedBy = constructionPriceItemExist.CreatedBy;
                        //            constructionPriceItem.CreatedById = constructionPriceItemExist.CreatedById;
                        //            constructionPriceItem.UpdatedBy = fullName;
                        //            constructionPriceItem.UpdatedById = userId;

                        //            _context.Update(constructionPriceItem);

                        //            constructionPriceItems.Remove(constructionPriceItemExist);
                        //        }
                        //    }
                        //}

                        //if (constructionPriceItems.Count > 0)
                        //{
                        //    constructionPriceItems.ForEach(item => {
                        //        item.UpdatedAt = DateTime.Now;
                        //        item.UpdatedById = userId;
                        //        item.UpdatedBy = fullName;
                        //        item.Status = AppEnums.EntityStatus.DELETED;
                        //    });
                        //    _context.UpdateRange(constructionPriceItems);
                        //}

                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa Chỉ số giá xây dựng công trình cho loại công trình " + input.NameOfConstruction, "ConstructionPrice", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!ConstructionPriceExists(data.Id))
                        {
        ...
        // DELETE: api/ConstructionPrice/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa Chỉ số giá xây dựng công trình cho loại công trình " + data.NameOfConstruction, "ConstructionPrice", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool ConstructionPriceExists(int id)
        {
            return _context.ConstructionPricies.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Construction_price;

                List<ConstructionPriceDataImport> data = new List<ConstructionPriceDataImport>();
                ...
                List<ConstructionPriceDataImport> dataValid = new List<ConstructionPriceDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.ConstructionPricies.AddRange(dataValid);
        ...
        public static List<ConstructionPriceDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<ConstructionPriceDataImport> res = new List<ConstructionPriceDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    ConstructionPriceDataImport input1Detai = new ConstructionPriceDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Chi tiết loại công trình chưa có dữ liệu \n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Chi tiết loại công trình\n";
                                }
                            }
                            else if (i == 4)
                            {
                                ...
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Căn cứ\n";
                                }
                            }
                            else if (i == 5)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.YearCompare = int.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Năm so với chưa có dữ liệu \n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Năm so với\n";
                                }
                            }
                            else if (i == 6)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Year = int.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Năm chưa có dữ liệu \n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Năm\n";
                                }
                            }
                            else if (i == 7)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<ConstructionPrice> data = _context.ConstructionPricies.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/chi_so_xay_dung_cong_trinh.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<ConstructionPrice> data, List<Decree> decrees)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.Des);

                                }
                                else if (i == 5)
                                {
                                    row.GetCell(i).SetCellValue(item.YearCompare);
                                }
                                else if (i == 6)
                                {
                                    row.GetCell(i).SetCellValue(item.Year);
                                }
                                else if (i == 7)
                                {
