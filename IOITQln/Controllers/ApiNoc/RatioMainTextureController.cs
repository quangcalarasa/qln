...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class RatioMainTextureController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("RatioMainTexture", "RatioMainTexture");
        private static string functionCode = "RATIO_MAINTEXTURE_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public RatioMainTextureController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<RatioMainTexture> data = _context.RatioMainTexturies.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<RatioMainTextureData> res = _mapper.Map<List<RatioMainTextureData>>(data.ToList());
                        foreach (RatioMainTextureData item in res)
                        {
                            RatioMainTexture ratioMainTexture = _context.RatioMainTexturies.Where(b => b.Id == item.ParentId).FirstOrDefault();
                            if (ratioMainTexture != null)
                            {
                                item.ParentName = ratioMainTexture.Name;
                            }
                        }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                RatioMainTexture data = await _context.RatioMainTexturies.FindAsync(id);
        ...
        // POST: api/RatioMainTexture
        [HttpPost]
        public async Task<IActionResult> Post(RatioMainTexture input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (RatioMainTexture)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.RatioMainTexturies.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới Tỷ lệ giá trị kết cấu chính " + input.Name, "RatioMainTexture", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (RatioMainTextureExists(input.Id))
                        {
        ...
        // PUT: api/RatioMainTexture/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RatioMainTexture input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                RatioMainTexture data = await _context.RatioMainTexturies.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa Tỷ lệ giá trị kết cấu chính " + input.Name, "RatioMainTexture", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!RatioMainTextureExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa Tỷ lệ giá trị kết cấu chính " + data.Name, "RatioMainTexture", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool RatioMainTextureExists(int id)
        {
            return _context.RatioMainTexturies.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Ratio_MainTexture;

                List<RatioMaintextureDataImport> data = new List<RatioMaintextureDataImport>();
                ...
                List<RatioMaintextureDataImport> dataValid = data.Where(e => e.Valid == true).ToList();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.RatioMainTexturies.AddRange(dataValid);
        ...
        public static List<RatioMaintextureDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<RatioMaintextureDataImport> res = new List<RatioMaintextureDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    RatioMaintextureDataImport input1Detai = new RatioMaintextureDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 1)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Chi tiết loại nhà ở chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Chi tiết loại nhà ở\n";
                                }
                            }
                            else if (i == 2)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure1 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Móng\n";
                                }
                            }
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure2 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Khung cột\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure3 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Tường\n";
                                }
                            }
                            else if (i == 5)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure4 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Nền,sàn\n";
                                }
                            }
                            else if (i == 6)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure5 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Khung cột đỡ mái\n";
                                }
                            }
                            else if (i == 7)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.TypeMainTexTure6 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị kết cấu chính của Mái\n";
                                }
                            }
                        }
                        catch (Exception ex)
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<RatioMainTexture> data = _context.RatioMainTexturies.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/ty_le_gia_tri_ket_cau_chinh.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<RatioMainTexture> data)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    if(item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure1);
                                }
                                else if (i == 3)
                                {
                                    if (item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure2);
                                }
                                else if (i == 4)
                                {
                                    if (item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure3);

                                }
                                else if (i == 5)
                                {
                                    if (item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure4);
                                }
                                else if (i == 6)
                                {
                                    if (item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure5);
                                }
                                else if (i == 7)
                                {
                                    if (item.TypeMainTexTure1 != null)
                                        row.GetCell(i).SetCellValue((double)item.TypeMainTexTure6);
                                }
                            }
                            rowStart++;
                            k++;
