...
namespace IOITQln.Controllers.ApiNoc
{
    ...
    public class PromissoryController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("Promissory", "Promissory");
        private static string functionCode = "RENT_FILE";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public PromissoryController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<NocReceipt> data = _context.NocReceipts.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                NocReceipt data = await _context.NocReceipts.FindAsync(id);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NocReceipt input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (NocReceipt)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                NocReceipt exist = _context.NocReceipts.Where(f => input.Number != null && f.Number == input.Number && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (exist != null)
                {
                    def.meta = new Meta(211, "STT phiếu đã tồn tại đã tồn tại!");
                    return Ok(def);
                }

                var dataDbest = _context.debts.Where(l => l.Code == input.Code && l.Status != AppEnums.EntityStatus.DELETED).OrderByDescending(p => p.CreatedAt).ToList();
                if(dataDbest != null)
                {
                    if(input.Action == 1)
                    {
                        dataDbest[0].SurplusBalance += input.Price;
                    }
                    else
                    {
                        dataDbest[0].SurplusBalance = dataDbest[0].SurplusBalance - input.Price;
                    }
                    _context.debts.Update(dataDbest[0]);
                }
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.NocReceipts.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới : " + input.Id, "NocReceipt", 0, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(Guid id, [FromBody] NocReceipt input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                NocReceipt data = await _context.NocReceipts.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                NocReceipt exist = _context.NocReceipts.Where(f => input.Number != null && f.Id != id && f.Number == input.Number && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (exist != null)
                {
                    def.meta = new Meta(211, "STT phiếu đã tồn tại đã tồn tại!");
                    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa : " + input.Id, "NocReceipt", 0, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa : " + data.Id, "NocReceipt", 0, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool NocReceiptExists(Guid id)
        {
            return _context.NocReceipts.Count(e => e.Id == id) > 0;
        }
    }
}
