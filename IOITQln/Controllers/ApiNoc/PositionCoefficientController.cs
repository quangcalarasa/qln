...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class PositionCoefficientController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("PositionCoefficient", "PositionCoefficient");
        private static string functionCode = "POSITION_COEFFICIENT_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public PositionCoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<PositionCoefficient> data = _context.PositionCoefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<PositionCoefficientData> res = _mapper.Map<List<PositionCoefficientData>>(data.ToList());
                        foreach (PositionCoefficientData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                PositionCoefficient data = await _context.PositionCoefficients.FindAsync(id);
        ...
        // POST: api/PositionCoefficient
        [HttpPost]
        public async Task<IActionResult> Post(PositionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (PositionCoefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.PositionCoefficients.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số vị trí " + input.Name, "PositionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (PositionCoefficientExists(input.Id))
                        {
        ...
        // PUT: api/PositionCoefficient/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PositionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                PositionCoefficient data = await _context.PositionCoefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa thông tin hệ số vị trí " + input.Name, "PositionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!PositionCoefficientExists(data.Id))
                        {
        ...
        // DELETE: api/PositionCoefficient/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa thông tin hệ số vị trí đất " + data.Name, "PositionCoefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool PositionCoefficientExists(int id)
        {
            return _context.PositionCoefficients.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Position_Coefficient;

                List<PositionCoefficientDataImport> data = new List<PositionCoefficientDataImport>();
                ...
                List<PositionCoefficientDataImport> dataValid = new List<PositionCoefficientDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.PositionCoefficients.AddRange(dataValid);
        ...
        public static List<PositionCoefficientDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<PositionCoefficientDataImport> res = new List<PositionCoefficientDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    PositionCoefficientDataImport input1Detai = new PositionCoefficientDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Áp dụng theo chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Áp dụng theo\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.DoApply = DateTime.FromOADate(Double.Parse(str));
                                        if (input1Detai.DoApply.Year < 1900)
                                        {
                                            input1Detai.Valid = false;
                                            input1Detai.ErrMsg += "Ngày áp dụng không hợp lệ\n";
                                        }
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Ngày áp dụng\n";
                                }
                            }
                            else if (i == 5)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.LocationValue1 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Vị trí 1 - Mặt tiền đường chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Vị trí 1 - Mặt tiền đường\n";
                                }
                            }
                            else if (i == 6)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.LocationValue2 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Vị trí 2 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Vị trí 2\n";
                                }
                            }
                            else if (i == 7)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.LocationValue3 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Vị trí 3 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Vị trí 3\n";
                                }
                            }
                            else if (i == 8)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.LocationValue4 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Vị trí 4 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Vị trí 4\n";
                                }
                            }
                            else if (i == 9)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyValue1 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hẻm vị trí 1 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm vị trí 1\n";
                                }
                            }
                            else if (i == 10)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyValue2 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hẻm vị trí 2 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm vị trí 2\n";
                                }
                            }
                            else if (i == 11)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyValue3 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hẻm vị trí 3 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm vị trí 3\n";
                                }
                            }
                            else if (i == 12)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyValue4 = float.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hẻm vị trí 4 chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm vị trí 4\n";
                                }
                            }
                            else if (i == 13)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyLevel2 = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm cấp 2\n";
                                }
                            }
                            else if (i == 14)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyOther = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm cấp còn lại\n";
                                }
                            }
                            else if (i == 15)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.AlleyLand = float.Parse(str);
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hẻm trải đất\n";
                                }
                            }
                            else if (i == 16)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.PositionDeep = float.Parse(str);
                                    }
                                }
                                catch
                                {
                            ...
                            else if (i == 17)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.LandPriceRefinement = float.Parse(str);
                                    }
                                }
                                catch
                                {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<PositionCoefficient> data = _context.PositionCoefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/he_so_vi_tri.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<PositionCoefficient> data, List<Decree> decrees)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.DoApply != null ? item.DoApply.ToString("dd/MM/yyyy") : "");
                                }
                                else if (i == 5)
                                {
                                    row.GetCell(i).SetCellValue(item.LocationValue1);
                                }
                                else if (i == 6)
                                {
                                    row.GetCell(i).SetCellValue(item.LocationValue2);
                                }
                                else if (i == 7)
                                {
                                    row.GetCell(i).SetCellValue(item.LocationValue3);
                                }
                                else if (i == 8)
                                {
                                    row.GetCell(i).SetCellValue(item.LocationValue4);
                                }
                                else if (i == 9)
                                {
                                    row.GetCell(i).SetCellValue(item.AlleyValue1);
                                }
                                else if (i == 10)
                                {
                                    row.GetCell(i).SetCellValue(item.AlleyValue2);
                                }
                                else if (i == 11)
                                {
                                    row.GetCell(i).SetCellValue(item.AlleyValue3);
                                }
                                else if (i == 12)
                                {
                                    row.GetCell(i).SetCellValue(item.AlleyValue4);
                                }
                                else if (i == 13)
                                {
                                    if(item.AlleyLevel2 != null)
                                        row.GetCell(i).SetCellValue((float)item.AlleyLevel2);
                                }
                                else if (i == 14)
                                {
                                    if (item.AlleyOther != null)
                                        row.GetCell(i).SetCellValue((float)item.AlleyOther);
                                }
                                else if (i == 15)
                                {
                                    if (item.AlleyLand != null)
                                        row.GetCell(i).SetCellValue((float)item.AlleyLand);
                                }
                                else if (i == 16)
                                {
                                    if (item.PositionDeep != null)
                                        row.GetCell(i).SetCellValue((float)item.PositionDeep);
                                }
                                else if (i == 17)
                                {
                                    if (item.LandPriceRefinement != null)
                                        row.GetCell(i).SetCellValue((float)item.LandPriceRefinement);
                                }
                            }
                            rowStart++;
                            k++;
