...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class AreaCorrectionCoefficientController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("AreaCorrectionCoefficient", "AreaCorrectionCoefficient");
        private static string functionCode = "AREA_CORRECTION_COEFFICIENT_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public AreaCorrectionCoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<AreaCorrectionCoefficient> data = _context.AreaCorrectionCoefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<AreaCorrectionCoefficientData> res = _mapper.Map<List<AreaCorrectionCoefficientData>>(data.ToList());
                        foreach(AreaCorrectionCoefficientData item in res)
                        {
                            AreaCorrectionCoefficient areaCorrectionCoefficient = _context.AreaCorrectionCoefficients.Where(p => p.Id == item.ParentId).FirstOrDefault();
                            item.ParentName = areaCorrectionCoefficient != null ? areaCorrectionCoefficient.Name : "";
        ...
        // GET: api/AreaCorrectionCoefficient/1
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                AreaCorrectionCoefficient data = await _context.AreaCorrectionCoefficients.FindAsync(id);
        ...
        // POST: api/AreaCorrectionCoefficient
        [HttpPost]
        public async Task<IActionResult> Post(AreaCorrectionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (AreaCorrectionCoefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.AreaCorrectionCoefficients.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới Hệ số điều chỉnh vùng ", "AreaCorrectionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (AreaCorrectionCoefficientExists(input.Id))
                        {
        ...
        // PUT: api/AreaCorrectionCoefficient/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AreaCorrectionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                AreaCorrectionCoefficient data = await _context.AreaCorrectionCoefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa hệ số điều chỉnh vùng ", "AreaCorrectionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!AreaCorrectionCoefficientExists(data.Id))
                        {
        ...
        // DELETE: api/AreaCorrectionCoefficient/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa hệ số điều chỉnh vùng", "AreaCorrectionCoefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool AreaCorrectionCoefficientExists(int id)
        {
            return _context.AreaCorrectionCoefficients.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Area_Correction_Coefficient;

                List<AreaCorrectionCoefficientDataImport> data = new List<AreaCorrectionCoefficientDataImport>();
                ...
                List<AreaCorrectionCoefficientDataImport> dataValid = new List<AreaCorrectionCoefficientDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.AreaCorrectionCoefficients.AddRange(dataValid);
        ...
        public static List<AreaCorrectionCoefficientDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<AreaCorrectionCoefficientDataImport> res = new List<AreaCorrectionCoefficientDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    AreaCorrectionCoefficientDataImport input1Detai = new AreaCorrectionCoefficientDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Vùng quy định chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Vùng quy định\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Chi tiết vùng quy định chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Chi tiết vùng quy định\n";
                                }
                            }
                            else if (i == 5)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hệ số điều chỉnh vùng cho Suất vốn đầu tư xây dựng công trình chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Hệ số điều chỉnh vùng cho Suất vốn đầu tư xây dựng công trình\n";
                                }
                            }
                            else if (i == 6)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.Value = double.Parse(str);
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hệ số điều chỉnh vùng chưa có dữ liệu \n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Hệ số điều chỉnh vùng\n";
                                }
                            }
                            else if (i == 7)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<AreaCorrectionCoefficient> data = _context.AreaCorrectionCoefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/he_so_dieu_chinh_vung.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<AreaCorrectionCoefficient> data, List<Decree> decrees)
        {
