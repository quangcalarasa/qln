...
namespace IOITQln.Controllers.Api
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("SalaryCoefficient", "SalaryCoefficient");
        private static string functionCode = "SALARY_COEFFICIENT_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public SalaryCoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<SalaryCoefficient> data = _context.SalaryCoefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<SalaryCoefficientData> res = _mapper.Map<List<SalaryCoefficientData>>(data.ToList());
                        foreach (SalaryCoefficientData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                SalaryCoefficient data = await _context.SalaryCoefficients.FindAsync(id);
        ...
        // POST: api/SalaryCoefficient
        [HttpPost]
        public async Task<IActionResult> Post(SalaryCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (SalaryCoefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.SalaryCoefficients.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số lương cơ bản ", "SalaryCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (SalaryCoefficientExists(input.Id))
                        {
        ...
        // PUT: api/SalaryCoefficient/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] SalaryCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                SalaryCoefficient data = await _context.SalaryCoefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa hệ số lương cơ bản ", "SalaryCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!SalaryCoefficientExists(data.Id))
                        {
        ...
        // DELETE: api/SalaryCoefficient/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa hệ số lương cơ bản", "SalaryCoefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> getByDecreeAndDate(SalaryCoefficientDto input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                SalaryCoefficient data = await _context.SalaryCoefficients.Where(s => ((s.DecreeType1Id == input.DecreeType1Id && input.DecreeType1Id != null) || input.DecreeType1Id == null) && s.DoApply <= input.DoApply && s.Status != AppEnums.EntityStatus.DELETED).OrderByDescending(s => s.DoApply).FirstOrDefaultAsync();
        ...
        {
            return _context.SalaryCoefficients.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Salary_Coefficient;

                List<SalaryCoefficientDataImport> data = new List<SalaryCoefficientDataImport>();
                ...
                List<SalaryCoefficientDataImport> dataValid = new List<SalaryCoefficientDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.SalaryCoefficients.AddRange(dataValid);
        ...
        public static List<SalaryCoefficientDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<SalaryCoefficientDataImport> res = new List<SalaryCoefficientDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    SalaryCoefficientDataImport input1Detai = new SalaryCoefficientDataImport();
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        try
                        {
                            ...
                            else if (i == 4)
                            {
                                ...
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Giá trị\n";
                                }
                            }
                            else if (i == 5)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<SalaryCoefficient> data = _context.SalaryCoefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/he_so_luong_co_ban.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<SalaryCoefficient> data, List<Decree> decrees)
        {
