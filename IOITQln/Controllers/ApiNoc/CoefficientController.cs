...
namespace IOITQln.Controllers.ApiNoc
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("coefficient", "coefficient");
        private readonly ApiDbContext _context;
        string functionCode = "COEFFICIENT";
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;
        public CoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Coefficient> data = _context.Coefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<CoefficientData> res = _mapper.Map<List<CoefficientData>>(data.ToList());
                        foreach (CoefficientData item in res)
                        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Coefficient data = await _context.Coefficients.FindAsync(id);
                ...
                def.meta = new Meta(200, ApiConstants.MessageResource.ACCTION_SUCCESS);
                CoefficientData res = _mapper.Map<CoefficientData>(data);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Coefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Coefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.Coefficients.Add(input);
                    try
                    {
                        await _context.SaveChangesAsync();
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới danh sách Hệ số thời điểm bố trí: " + input.Id, "Coefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/Coefficient/Put
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Coefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Coefficient data = await _context.Coefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa danh sách Hệ số thời điểm bố trí: " + input.Id, "Coefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa danh sách Hệ số thời điểm bố trí: " + data.Id, "Coefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        #region export hệ số thời điểm bố trí
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<Coefficient> coefficients = _context.Coefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<CoefficientData> mapper_data = _mapper.Map<List<CoefficientData>>(coefficients);
            ...
            string template = @"NOCexcel/hesothoidiembotri.xlsx";
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<CoefficientData> data)
        {
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.NocCoefficient;

                List<CoefficientData> data = new List<CoefficientData>();
                ...
                List<CoefficientData> dataValid = new List<CoefficientData>();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.Coefficients.AddRange(dataValid);
        ...
        public static List<CoefficientData> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<CoefficientData> res = new List<CoefficientData>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    CoefficientData inputDetail = new CoefficientData();
