...
namespace IOITQln.Controllers.Api
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("Decree", "Decree");
        private static string functionCodeType1 = "DECREE_TYPE1_MANAGEMENT";
        private static string functionCodeType2 = "DECREE_TYPE2_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public DecreeController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        [HttpGet("GetByPage/{type}")]
        public IActionResult GetByPage([FromQuery] FilteredPagination paging, int type)
        {
            ...
            string functionCode = type == 1 ? functionCodeType1 : functionCodeType2;
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Decree> data = _context.Decreies.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        // GET: api/Decree/1
        [HttpGet("{type}/{id}")]
        public async Task<IActionResult> GetById(int type, int id)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                Decree data = await _context.Decreies.FindAsync(id);
        ...
        // POST: api/Decree
        [HttpPost("{type}")]
        public async Task<IActionResult> Post(int type, [FromBody] Decree input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Decree)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                Decree codeExist = _context.Decreies.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới " + (input.TypeDecree == AppEnums.TypeDecree.NGHI_DINH ? "nghị định" : "thông tư, vản bản quyết định") + " " + input.Code, "Decree", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        // PUT: api/Decree/1
        [HttpPut("{type}/{id}")]
        public async Task<IActionResult> Put(int type, int id, [FromBody] Decree input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Decree data = await _context.Decreies.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                Decree codeExist = _context.Decreies.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                if (codeExist != null)
                {
                    def.meta = new Meta(211, "Mã đã tồn tại!");
                    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa " + (input.TypeDecree == AppEnums.TypeDecree.NGHI_DINH ? "nghị định" : "thông tư, vản bản quyết định") + " " + input.Code, "Decree", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        // DELETE: api/Decree/1
        [HttpDelete("{type}/{id}")]
        public async Task<IActionResult> Delete(int type, int id)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa " + (data.TypeDecree == AppEnums.TypeDecree.NGHI_DINH ? "nghị định" : "thông tư, vản bản quyết định") + " " + data.Code, "Decree", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool DecreeExists(int id)
        {
            return _context.Decreies.Count(e => e.Id == id) > 0;
        }
        ...
        //    string fullName = identity.Claims.Where(c => c.Type == "FullName").Select(c => c.Value).SingleOrDefault() ?? string.Empty;
        //    string access_key = identity.Claims.Where(c => c.Type == "AccessKey").Select(c => c.Value).SingleOrDefault();
        //    if (!CheckRole.CheckRoleByCode(access_key, functionCodeType2, (int)AppEnums.Action.IMPORT))
        //    {
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.NocDecreeType2;

                List<DecreeType2DataImport> data = new List<DecreeType2DataImport>();
                ...
                List<DecreeType2DataImport> dataValid = data.Where(e => e.Valid == true).ToList();
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        dataValid.ForEach(x => x.TypeDecree = AppEnums.TypeDecree.THONG_TU_VAN_BAN);
                        ...
                        _context.Decreies.AddRange(dataValid);
        ...
        public static List<DecreeType2DataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<DecreeType2DataImport> res = new List<DecreeType2DataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    DecreeType2DataImport input1Detai = new DecreeType2DataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 1)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Văn bản số chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Văn bản số\n";
                                }
                            }
                            else if (i == 2)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.ApplyCalculateRentalPrice = true;
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Áp dụng tính giá thuê\n";
                                }
                            }
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.DoPub = DateTime.FromOADate(Double.Parse(str));
                                        if (input1Detai.DoPub.Value.Year < 1900)
                                        {
                                            input1Detai.DoPub = null;
                                            input1Detai.Valid = false;
                                            input1Detai.ErrMsg += "Ngày ban hành không hợp lệ\n";
                                        }
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Ngày ban hành\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.DecisionUnit = str;
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Cơ quan ban hành\n";
                                }
                            }
                            else if (i == 5)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            if (!CheckRole.CheckRoleByCode(access_key, functionCodeType2, (int)AppEnums.Action.EXPORT))
            {
                return new FileContentResult(new byte[0], "application/octet-stream");
            }

            List<Decree> data = _context.Decreies.Where(p => p.TypeDecree == AppEnums.TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/van_ban_phap_luat_lien_quan.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<Decree> data)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.ApplyCalculateRentalPrice == true ? "true" : "");
                                }
                                else if (i == 3)
                                {
                                    row.GetCell(i).SetCellValue(item.DoPub != null ? item.DoPub.Value.ToString("dd/MM/yyyy") : "");
                                }
                                else if (i == 4)
                                {
                                    row.GetCell(i).SetCellValue(item.DecisionUnit);
