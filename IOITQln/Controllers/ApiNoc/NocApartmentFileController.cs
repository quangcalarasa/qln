...
namespace IOITQln.Controllers.ApiNoc
{
    ...
    [ApiController]
    public class NocApartmentFileController : ControllerBase
    {
        ...
        public NocApartmentFileController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<NocApartmentFile> data = _context.NocApartmentFiles.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NocApartmentFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (NocApartmentFile)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                List<NocApartmentFile> item = _context.NocApartmentFiles.Where(l => l.Status != AppEnums.EntityStatus.DELETED).ToList();

                Apartment apartment = _context.Apartments.Where(x => x.Id == input.NocApartmentId).FirstOrDefault();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.NocApartmentFiles.Add(input);
                    ...
                    try
                    {
                        ...
                        LogActionModel logActionModel = new LogActionModel("Thêm mới tệp đính kèm: " + input.Id, "NocApartmentFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (NocApartmentFileExists(input.Id))
                        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NocApartmentFile input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                NocApartmentFile data = await _context.NocApartmentFiles.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa : " + input.Id, "NocApartmentFile", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!NocApartmentFileExists(data.Id))
                        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                NocApartmentFile data = await _context.NocApartmentFiles.FindAsync(id);
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa " + data.Id, "NocApartmentFile", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        [HttpGet("getFile/{nocApartmentId}")]
        public async Task<IActionResult> GetPayBynocApartmentId(int nocApartmentId)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                var data = await _context.NocApartmentFiles.Where(f => f.NocApartmentId == nocApartmentId && f.Status != AppEnums.EntityStatus.DELETED).ToListAsync();
                if (data == null)
                {
        ...
        private bool NocApartmentFileExists(int id)
        {
