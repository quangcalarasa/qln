...
namespace IOITQln.Controllers.ApiNoc
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("DEBTS", "DEBTS");
        private static string functionCode = "DEBTS";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;

        public DebtsController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<Debts> data = _context.debts.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                Debts data = await _context.debts.FindAsync(id);
        ...
        [HttpPost("Debts/{Id}")]
        public async Task<IActionResult> Post(Guid id, [FromBody] Debts input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (Debts)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        List<Debts> lst = _context.debts.Where(x => x.RentFileId == id && x.Status != AppEnums.EntityStatus.DELETED).ToList();
                        ...
                        _context.debts.Add(input);

                        await _context.SaveChangesAsync();
                        //thêm LogAction

                        LogActionModel logActionModel = new LogActionModel("Thêm mới : " + input.Id, "Debts", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Debts input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                Debts data = await _context.debts.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if (data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa : " + input.Id, "Debts", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa công nợ: " + data.Id, "Debts", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        {
            return _context.debts.Count(e => e.Id == id) > 0;
        }
        ...
        public async Task<IActionResult> getDataByCode(string code)
        {
            ...
            try
            {
                var block = _context.Blocks.Where(p => p.Code == code && p.TypeBlockEntity == AppEnums.TypeBlockEntity.BLOCK_RENT && p.Status != AppEnums.EntityStatus.DELETED).Select(p => p.Code).FirstOrDefault();
                var apartment = _context.Apartments.Where(p => p.Code == code && p.TypeApartmentEntity == AppEnums.TypeApartmentEntity.APARTMENT_RENT && p.Status != AppEnums.EntityStatus.DELETED).Select(p => p.Code).FirstOrDefault();

                if (block == null && apartment == null)
                {
                    def.meta = new Meta(200, "Không tìm thấy căn nhà (căn hộ) với mã định danh tương ứng");
                    return Ok(def);
                }
                def.meta = new Meta(200, ApiConstants.MessageResource.ACCTION_SUCCESS);
                if (block != null)
