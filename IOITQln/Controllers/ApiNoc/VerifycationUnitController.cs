...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class VerifycationUnitController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("verifycation-unit", "verifycation-unit");
        private static string functionCode = "VERIFYCATION_UNIT_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;

        public VerifycationUnitController(ApiDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<VerifycationUnit> data = _context.VerifycationUnits.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                VerifycationUnit data = await _context.VerifycationUnits.FindAsync(id);
        ...
        // POST: api/VerifycationUnit
        [HttpPost]
        public async Task<IActionResult> Post(VerifycationUnit input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (VerifycationUnit)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                VerifycationUnit codeExist = _context.VerifycationUnits.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED).FirstOrDefault();
                if (codeExist != null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.VerifycationUnits.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới cơ quan xác minh " + input.Name, "VerifycationUnit", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (VerifycationUnitExists(input.Id))
                        {
        ...
        // PUT: api/VerifycationUnit/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] VerifycationUnit input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                VerifycationUnit data = await _context.VerifycationUnits.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                VerifycationUnit codeExist = _context.VerifycationUnits.Where(f => f.Code == input.Code && f.Status != AppEnums.EntityStatus.DELETED && f.Id != id).FirstOrDefault();
                if (codeExist != null)
                {
                    def.meta = new Meta(211, "Mã đã tồn tại!");
                    return Ok(def);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa cơ quan xác minh " + input.Name, "VerifycationUnit", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!VerifycationUnitExists(data.Id))
                        {
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa cơ quan xác minh " + data.Name, "VerifycationUnit", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool VerifycationUnitExists(int id)
        {
            return _context.VerifycationUnits.Count(e => e.Id == id) > 0;
        }
    }
}
