...
namespace IOITQln.Controllers.Api
{
    ...
    [ApiController]
    public class DeductionCoefficientController : ControllerBase
    {
        private static readonly ILog log = LogMaster.GetLogger("DeductionCoefficient", "DeductionCoefficient");
        private static string functionCode = "DEDUCTION_COEFFICIENT_MANAGEMENT";
        private readonly ApiDbContext _context;
        private IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;
        private readonly IConfiguration _configuration;

        public DeductionCoefficientController(ApiDbContext context, IMapper mapper, IWebHostEnvironment hostingEnvironment, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            try
            {
                if (paging != null)
                {
                    ...
                    IQueryable<DeductionCoefficient> data = _context.DeductionCoefficients.Where(c => c.Status != AppEnums.EntityStatus.DELETED);
                    ...
                    {
                        List<DeductionCoefficientData> res = _mapper.Map<List<DeductionCoefficientData>>(data.ToList());
                        foreach(DeductionCoefficientData item in res)
                        {
        ...
        // GET: api/DeductionCoefficient/1
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            ...
            try
            {
                DeductionCoefficient data = await _context.DeductionCoefficients.FindAsync(id);
        ...
        // POST: api/DeductionCoefficient
        [HttpPost]
        public async Task<IActionResult> Post(DeductionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                input = (DeductionCoefficient)UtilsService.TrimStringPropertyTypeObject(input);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _context.DeductionCoefficients.Add(input);
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Thêm mới hệ số miễn giảm ", "DeductionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.CREATE, userId, fullName);
                    ...
                    catch (DbUpdateException e)
                    {
                        ...
                        transaction.Rollback();
                        if (DeductionCoefficientExists(input.Id))
                        {
        ...
        // PUT: api/DeductionCoefficient/1
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] DeductionCoefficient input)
        {
            string accessToken = Request.Headers[HeaderNames.Authorization];
            Entities.Token token = (from t in _context.Tokens where Convert.ToString(t.AccessToken) == accessToken select t).FirstOrDefault();
            if (token == null)
            ...
            try
            {
                ...
                DeductionCoefficient data = await _context.DeductionCoefficients.AsNoTracking().Where(e => e.Id == id && e.Status != AppEnums.EntityStatus.DELETED).FirstOrDefaultAsync();
                if(data == null)
                {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Sửa hệ số miễn giảm ", "DeductionCoefficient", input.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(input), (int)AppEnums.Action.UPDATE, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
                    ...
                    catch (DbUpdateException e)
                    {
                        transaction.Rollback();
                        log.Error("DbUpdateException:" + e);
                        if (!DeductionCoefficientExists(data.Id))
                        {
        ...
        // DELETE: api/DeductionCoefficient/1
        [HttpDelete("{id}")]
            ...
            try
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        //thêm LogAction
                        LogActionModel logActionModel = new LogActionModel("Xóa hệ số miễn giảm", "DeductionCoefficient", data.Id, HttpContext.Connection.RemoteIpAddress.ToString(), JsonConvert.SerializeObject(data), (int)AppEnums.Action.DELETED, userId, fullName);
                        LogAction logAction = _mapper.Map<LogAction>(logActionModel);
                        _context.Add(logAction);
                        await _context.SaveChangesAsync();
        ...
        private bool DeductionCoefficientExists(int id)
        {
            return _context.DeductionCoefficients.Count(e => e.Id == id) > 0;
        }
        ...
        public IActionResult ImportDataExcel()
        {
            ...
            try
            {
                ...
                importHistory.Type = AppEnums.ImportHistoryType.Noc_Deduction_Coefficient;

                List<DeductionCoefficientDataImport> data = new List<DeductionCoefficientDataImport>();
                ...
                List<DeductionCoefficientDataImport> dataValid = new List<DeductionCoefficientDataImport>();
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        _context.DeductionCoefficients.AddRange(dataValid);
        ...
        public static List<DeductionCoefficientDataImport> importData(MemoryStream ms, int sheetnumber, int rowStart)
        {
            XSSFWorkbook workbook = new XSSFWorkbook(ms);
            ISheet sheet = workbook.GetSheetAt(sheetnumber);

            List<DeductionCoefficientDataImport> res = new List<DeductionCoefficientDataImport>();
            for (int row = rowStart; row <= sheet.LastRowNum; row++)
            {
                if (sheet.GetRow(row) != null)
                {
                    //Đọc dữ liệu từ từng cell
                    DeductionCoefficientDataImport input1Detai = new DeductionCoefficientDataImport();
                        ...
                        try
                        {
                            ...
                            else if (i == 3)
                            {
                                try
                                {
                                    if (str != "")
                                    {
                                        input1Detai.ObjectApply = str;
                                    }
                                    else
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Đối tượng miễn giảm chưa có dữ liệu\n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột Đối tượng miễn giảm\n";
                                }
                            }
                            else if (i == 4)
                            {
                                try
                                {
                                    ...
                                    {
                                        input1Detai.Valid = false;
                                        input1Detai.ErrMsg += "Cột Hệ số được miễn giảm \n";
                                    }
                                }
                                catch
                                {
                                    input1Detai.Valid = false;
                                    input1Detai.ErrMsg += "Lỗi cột thông tin Hệ số được miễn giảm\n";
                                }
                            }
                            else if (i == 5)
                            {
        ...
        public async Task<IActionResult> ExportExcel()
        {
            ...
            List<DeductionCoefficient> data = _context.DeductionCoefficients.Where(p => p.Status != AppEnums.EntityStatus.DELETED).ToList();
            List<Decree> decrees = _context.Decreies.Where(p => p.TypeDecree == TypeDecree.THONG_TU_VAN_BAN && p.Status != AppEnums.EntityStatus.DELETED).ToList();
            ...
            string template = @"NOCexcel/he_so_mien_giam_tien_nha.xlsx";
            string webRootPath = _hostingEnvironment.WebRootPath;
        ...
        private static MemoryStream WriteDataToExcel(string templatePath, int sheetnumber, List<DeductionCoefficient> data, List<Decree> decrees)
        {
                    ...
                    foreach (var item in data)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < datacol; i++)
                            {
                                ...
                                {
                                    row.GetCell(i).SetCellValue(item.ObjectApply);
                                }
                                else if (i == 4)
                                {
